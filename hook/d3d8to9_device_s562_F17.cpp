...
void Direct3DDevice8::AqVorZeichnen()
{
	...
	{
		if (AqVS3 && AqVSLicht && CurrentVertexShaderHandle != 0
			&& CurrentPixelShaderHandle == 0)
		{
	...
	if (!Zustaendig)
	{
		...
		// aufgebaut werden.
		if (Gewuenscht == nullptr && AqVS3 && AqVSLicht && CurrentVertexShaderHandle != 0)
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
		...
		// ---- Modus 5: der Blickvektor, einmal je Bildpunkt ----
		// V = normalize(Auge - Ort). Beides im Objektraum, deshalb ohne
		// jede Matrix. r7 haelt V, r9 sammelt den Glanz.
		//
		// *** NUR MODUS 5, NICHT ">= 5". *** Modus 6 ist die reine
		// ps_3_0-Uebersetzung von Modus 4, ohne Glanz und Fresnel. Sie
		// trennt die beiden Aenderungen, die sonst gemeinsam auftreten:
		// Registerumstellung einerseits, neue Beleuchtungsrechnung
		// andererseits (556.2).
		if (k.Modus == 5)
		{
			Rumpf += "    add r7.xyz, c0, -t6\n"
