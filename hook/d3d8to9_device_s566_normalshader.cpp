...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::CreateVertexShader(const DWORD *pDeclaration, const DWORD *pFunction, DWORD *pHandle, DWORD Usage)
{
	...
	*pHandle = 0;
	...
	if (pFunction != nullptr)
	{
		...
		AqWurdeVS3 = false;
		if (AqVS3 && AqLicht && AqNachVS3(SourceCode))
		{
#ifndef D3D8TO9NOLOG
			AqWurdeVS3 = true;
#ifndef D3D8TO9NOLOG
...
void Direct3DDevice8::AqVorZeichnen()
{
	...
	// *** MESSUNG STATT VERMUTUNG (F20). *** Die drei Zahlen sagen,
	// ob der Notbehelf ueberhaupt greift oder ob Objekte an ihm
	// vorbeilaufen. Alle 4000 Zeichenaufrufe eine Zeile -- oft genug
	// zum Sehen, selten genug fuer die Platte.
	if (AqVS3Aktiv)
	{
		++AqZaehlVS3Draw;
#ifndef D3D8TO9NOLOG
		if ((AqZaehlVS3Draw % 4000) == 0)
			LOG << "> AquaNox-MESSUNG: vs_3_0-Zeichenaufrufe " << AqZaehlVS3Draw
			    << ", Notbehelf " << AqZaehlNotPS
			    << ", Normalkarten gebunden " << AqNormalGebunden << ", OHNE Pixelshader " << AqZaehlOhnePS
			    << "  [A Schluessel=" << AqWegA
			    << " B Erzeugung=" << AqWegB
			    << " C nicht zustaendig=" << AqWegC << " C-Nachbau=" << AqWegC2 << "]"
			    << "  {C-gesamt=" << AqFlagGesamt
			    << " kein Schatten=" << AqFlagKeinSchatten
			    << " kein VS3Aktiv=" << AqFlagKeinVS3Aktiv
			    << " kein VSLicht=" << AqFlagKeinLicht << "}" << std::endl;
#endif
	}
	...
	if (!AqSchattenGueltig)
	{
			...
			&& CurrentPixelShaderHandle == 0)
		{
			...
			++AqZaehlNotPS;
			++AqWegA;
			if (Not != nullptr && AqAktiverPS != Not)
			{
	...
	if (!Zustaendig)
	{
		...
		// *** MESSUNG STATT DRITTER VARIANTE (F23). *** Die Rettung
		// oben greift nie (C-Nachbau=0). Hier wird gezaehlt, WELCHE
		// Bedingung sie verhindert -- einzeln, nicht geraten.
		if (Gewuenscht == nullptr && AqVS3Aktiv && CurrentVertexShaderHandle != 0)
		{
			++AqFlagGesamt;
			if (!AqSchattenGueltig) ++AqFlagKeinSchatten;
			if (!AqVS3Aktiv)        ++AqFlagKeinVS3Aktiv;
			if (!AqVSLicht)         ++AqFlagKeinLicht;
		}

		if (Gewuenscht == nullptr && AqVS3Aktiv
			&& CurrentVertexShaderHandle != 0 && AqSchattenGueltig)
		{
			AqSchluessel kn;
	...
	// *** DIE ZWEITE STELLE, AN DER EIN vs_3_0 OHNE PARTNER BLIEBE
	// (F17b). *** Der Schluessel war hier nachbaubar, aber die
	// Erzeugung schlug fehl -- Budget, Assembler, CreatePixelShader.
	// Beim ersten Anlauf hatte ich nur den Fall "Schluessel nicht
	// baubar" abgefangen; dieser hier blieb offen, und deshalb
	// verschwanden die Pflanzen weiterhin.
...
bool Direct3DDevice8::AqBaueSchluessel(AqSchluessel &k) const
{
	...
	for (DWORD s = 0; s < 4; ++s)
	{
		...
		// falsches Bild (547.7, Falle C).
		if (AqTSS[s][D3DTSS_TEXCOORDINDEX] != s)          return false;
		if (AqTSS[s][D3DTSS_RESULTARG] != D3DTA_CURRENT)  return false;
		...
		if (AqTSS[s][D3DTSS_TEXTURETRANSFORMFLAGS] != D3DTTFF_DISABLE) return false;
		...
		if (CA1 > 2 || CA2 > 2 || AA1 > 2 || AA2 > 2)     return false;
		...
		if (COp != D3DTOP_SELECTARG1 && COp != D3DTOP_SELECTARG2 &&
			...
			COp != D3DTOP_MODULATEALPHA_ADDCOLOR)         return false;

		if (AOp != D3DTOP_SELECTARG1 && AOp != D3DTOP_SELECTARG2 &&
			AOp != D3DTOP_MODULATE)                       return false;

		// Braucht die Stufe eine Textur, muss auch eine 2D gebunden sein.
		...
		if (BrauchtTextur && AqTexTyp[s] != 1)            return false;
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
		...
		if (k.NormalKarte)
			Rumpf += "    texld r11, t0, s4\n"
			         "    add r11.xy, r11, -c31.y\n"
			         "    mad r0.xy, r11, c31.z, r0\n"
			         "    dp3 r0.w, r0, r0\n"
			         "    max r0.w, r0.w, c31.x\n"
			         "    rsq r0.w, r0.w\n"
			         "    mul r0.xyz, r0, r0.w\n";
	...
	if (k.Modus == 5 && k.Lichter == 0 && k.Hemi)
		Rumpf += "    add_sat r3.xyz, r3, r9\n";

	if (k.Modus == 5 && k.Lichter > 0)
	{
