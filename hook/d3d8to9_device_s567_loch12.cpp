...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetTexture(DWORD Stage, IDirect3DBaseTexture8 *pTexture)
{
	...
	case D3DRTYPE_TEXTURE:
	{
		...
		if (!T8->AqNormalGesucht && T8->AqHash != 0 && AqModus != 0
		...
		if (Stage == 0)
		{
			ProxyInterface->SetTexture(4, T8->AqNormal);
	...
	*pHandle = 0;
	...
	if (pFunction != nullptr)
	{
		...
		if (AqModus != 0)
		{
			...
			if (AqLicht && AqModus >= 3)
			{
				// *** DIE GRENZE IST 124, NICHT 128. *** Bei 128 kommentiert
				// der m*x*-Fix weiter unten "m4x4 oPos" aus -- das Objekt
				// verschwaende dann spurlos aus dem Bild.
				// *** NICHT DIE ZAHL AUS DEM KOMMENTAR NEHMEN. *** Sie
				// beschreibt den ORIGINALSHADER. Der Wrapper hat oben
				// bereits bis zu 22 Initialisierungs-mov eingefuegt und
				// in InstructionCount mitgezaehlt -- nur die ist aktuell
				// (F8).
				...
				{
					// oT5..oT7, NICHT oT3/oT4: t3 ist im Pixelshader zwingend
					// die Texturkoordinate der Stufe 3. So bleiben Objekte
					// mit bis zu fuenf Texturstufen nachbaubar.
				...
				{
					LOG << "> AquaNox: kein Platz (" << Slots << " Slots), Shader bleibt unveraendert" << std::endl;
				}
#endif
		...
		if (AqVS3 && AqLicht)
		{
			if (AqNachVS3(SourceCode))
			{
				AqWurdeVS3 = true;
#ifndef D3D8TO9NOLOG
				LOG << "> AquaNox: Vertexshader auf vs_3_0 umgestellt" << std::endl;
#endif
			}
			else
			{
				// *** BISHER STILL (567.3). *** Ein lichtrechnender
				// Shader, dessen Umstellung scheitert, bleibt vs_1_1 --
				// bekommt aber spaeter trotzdem unsere ps_3_0-Shader
				// gebunden: ungueltiges Paar, Objekt unsichtbar.
				// Genau die Fehlerklasse von F12/F16.
				++AqVS3Fehlgeschlagen;
#ifndef D3D8TO9NOLOG
				LOG << "> AquaNox: vs_3_0-UMSTELLUNG FEHLGESCHLAGEN -- bleibt vs_1_1" << std::endl;
#endif
			}
		}
	...
	ShaderInfo->IstVS3 = AqWurdeVS3;   // F19
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetVertexShader(DWORD Handle)
{
	...
	else
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			AqVS3Aktiv = ShaderInfo->IstVS3;   // F19
		}
	}
...
void Direct3DDevice8::AqVorZeichnen()
{
	...
	// *** DER VIERTE WEG (F17d). *** Nach einem Zustandsblock ist der
	// Schatten ungueltig, und diese Funktion kehrte hier sofort zurueck
	// -- ohne irgendeinen Pixelshader zu setzen. Bei vs_1_1 war das
	// harmlos. Bei vs_3_0 bleibt das Objekt ohne Partner und
	// verschwindet.
	//
	// Sichtbar wurde das erst in der ENTFERNUNG: aus der Naehe war es
	// nach F17a..c behoben, weiter weg nicht. Zustandsbloecke werden
	// offenbar beim Wechsel der Darstellungsart benutzt.
	if (!AqSchattenGueltig)
	{
		if (AqVS3Aktiv && CurrentVertexShaderHandle != 0
			&& CurrentPixelShaderHandle == 0)
		{
			IDirect3DPixelShader9 *const Not = AqHoleNotPS();
			if (Not != nullptr)
			{
				// Gezaehlt wird JEDER Zeichenaufruf, der mit dem
				// Notbehelf endet -- nicht nur der Bindungswechsel.
				// Nur so ist der Anteil an der Szene ablesbar.
				++AqNotD;
				if (AqAktiverPS != Not)
				{
					ProxyInterface->SetPixelShader(Not);
					AqAktiverPS = Not;
				}
			}
		}
	...
	// *** DIE ZUSTAENDIGKEITSREGEL. *** Wir greifen nur ein, wenn ein
	// lichtrechnender Vertexshader gebunden ist UND das Spiel selbst
	// keinen Pixelshader gesetzt hat. Alles andere -- Terrain, Wasser,
	// Pflanzen, HUD, Menue, Filme -- laeuft unveraendert weiter.
	const bool Zustaendig =
		AqVSLicht && CurrentVertexShaderHandle != 0 && CurrentPixelShaderHandle == 0;

	if (!Zustaendig)
	{
		// Unseren Shader wieder abraeumen, falls er noch am Geraet haengt.
		IDirect3DPixelShader9 *Gewuenscht =
			reinterpret_cast<IDirect3DPixelShader9 *>(CurrentPixelShaderHandle);

		// *** LOCH 2 (567.3): SPIELEIGENER PIXELSHADER AUF EINEM
		// vs_3_0-OBJEKT. *** Die zehn ps_1_x-Programme des Spiels sind
		// mit vs_3_0 ein ungueltiges Paar -- das Objekt verschwiende
		// lautlos, und keiner der bisherigen Zaehler saehe es.
		if (AqVS3Aktiv && Gewuenscht != nullptr)
		{
			++AqFremdPS;
		}

		// *** DRITTE STELLE (F17c). *** Wird hier auf "kein
		// Pixelshader" zurueckgeraeumt, obwohl der Vertexshader auf
		// vs_3_0 steht, bleibt das Objekt wieder ohne Partner. Das
		// betrifft genau den Fall, den der Nutzer beschrieben hat:
		// Objekte, die beim Wiedereintritt ins Sichtfeld neu
		// aufgebaut werden.
		// *** HIER LIEGEN 89 PROZENT DER FAELLE (F22). *** Gemessen:
		// 1.230.173 von 1.382.172 Notbehelf-Einsaetzen kommen aus
		// diesem Zweig -- Objekte, die ueber die Fixed-Function-
		// Pipeline gezeichnet werden sollen, deren Vertexshader aber
		// auf vs_3_0 steht. Fixed Function ist dann nicht mehr
		// erlaubt, also MUSS ein Pixelshader her.
		//
		// Der primitive Notbehelf ist dafuer das falsche Werkzeug: er
		// zeichnet Textur mal Farbe und ignoriert die Texturkette.
		// Richtig ist der NACHBAU, den es laengst gibt -- Modus 2,
		// der den Nachbau-Test bestanden hat (553.1). Er bildet
		// genau das nach, was Fixed Function hier taete.
		// Ein umgestellter Vertexshader braucht IMMER einen Pixelshader
		// (F16) -- auch hier, wo das Spiel selbst keinen gesetzt hat.
		// Erst der originalgetreue Nachbau (Modus 2), sonst der
		// Notbehelf.
		if (Gewuenscht == nullptr && AqVS3Aktiv && CurrentVertexShaderHandle != 0)
		{
			AqSchluessel kn;
			if (AqBaueSchluessel(kn))
			{
				kn.Modus = 2;   // reiner Nachbau, ohne eigene Beleuchtung
				kn.NormalKarte = 0;
				IDirect3DPixelShader9 *const Nach = AqHolePS(kn);
				if (Nach != nullptr)
				{
					++AqNachbauC;
					if (AqAktiverPS != Nach)
					{
						ProxyInterface->SetPixelShader(Nach);
						AqAktiverPS = Nach;
					}
					return;
				}
			...
			// *** HIER SASS DER ZAEHLER-FEHLER (F26): ein if OHNE
			// KLAMMERN, an das eine Textersetzung zwei Zaehlzeilen
			// angehaengt hatte. Nur die Zuweisung war bedingt, die
			// Zaehler liefen bei JEDEM Zeichenaufruf der Szene --
			// daher die erlogenen Millionen in 564/566. ***
			Gewuenscht = AqHoleNotPS();
			if (Gewuenscht != nullptr)
				++AqNotC;
		}
	...
	// ------------------------------------------------------------------
	// Modus 2, 3, 4: einen Pixelshader binden.
	// ------------------------------------------------------------------
	AqSchluessel k;
	if (!AqBaueSchluessel(k))
	{
		// *** MIT vs_3_0 IST "GAR NICHTS TUN" TOEDLICH (F17). ***
		// Bei vs_1_1 uebernimmt hier die Fixed-Function-Pipeline und
		// das Objekt sieht unveraendert aus. Ein vs_3_0 OHNE
		// Pixelshader ist dagegen ungueltig -- das Objekt verschwindet
		// lautlos. Sichtbar wurde das an Bodenpflanzen, die beim
		// Wegdrehen verschwanden und beim Hinsehen wiederkamen: sie
		// wechseln zwischen nachbaubaren und nicht nachbaubaren Ketten.
		//
		// Statt nichts zu binden, kommt daher ein Notbehelf: Textur 0
		// mal Diffusfarbe. Das ist nicht die Fixed-Function-Kette,
		// aber es ist SICHTBAR und nah genug.
		if (AqVS3Aktiv)
		{
			IDirect3DPixelShader9 *const Not = AqHoleNotPS();
			if (Not != nullptr)
		...
		// Nicht nachbaubar -> gar nichts tun. Das Objekt laeuft dann
		// unveraendert ueber die Fixed-Function-Pipeline.
		if (AqVS3Aktiv)
		{
			++AqOhnePartner;   // vs_3_0 ohne Pixelshader: der toedliche Fall
		}
	...
	IDirect3DPixelShader9 *PS = AqHolePS(k);

	// (F17b) Der Schluessel war baubar, aber die Erzeugung schlug fehl
	// -- Budget, Assembler, CreatePixelShader. *** AUCH HIER SASS DER
	// KLAMMERFEHLER F26: die Zaehler liefen bei jedem zustaendigen
	// Zeichenaufruf, weshalb "B Erzeugung" wie 100 Prozent aussah. ***
	if (PS == nullptr && AqVS3Aktiv)
	{
		PS = AqHoleNotPS();
		if (PS != nullptr)
			++AqNotB;
...
static const char *AqVSEingangSemantik(unsigned Reg)
{
	...
	case 16: return "dcl_position1";
	case 17: return "dcl_normal1";
	default: break;
	}
...
bool Direct3DDevice8::AqBaueSchluessel(AqSchluessel &k) const
{
	...
	k.NormalKarte = AqNormalAktiv ? 1 : 0;
	...
	for (DWORD s = 0; s < 4; ++s)
	{
		...
		// Nur DIFFUSE(0), CURRENT(1), TEXTURE(2) und keine Modifikatoren
		// (COMPLEMENT 0x10, ALPHAREPLICATE 0x20) -- gemessen reicht das.
		if (CA1 > 2 || CA2 > 2 || AA1 > 2 || AA2 > 2)
		{
		...
		//   Tex == 0 (gar keine): *** NICHT MEHR ABLEHNEN (F28). ***
		//     Fixed Function liefert implizit Weiss; der Generator
		//     bildet das jetzt mit "mov r6, c31.w" nach. Die alte
		//     Ablehnung schickte diese Ketten bei vs_3_0 in den
		//     Notbehelf -- die verschwindenden Bodenpflanzen.
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
		...
		if (k.NormalKarte)
		{
			...
			if (k.Modus != 14)
			{
				// *** MODUS 15: STOERUNG 24-FACH. *** Physikalisch
				// unsinnig, aber unuebersehbar. Zeigt Modus 14 die
				// Karte und hier passiert trotzdem nichts, liegt es
				// an der Verrechnung, nicht am Ankommen.
				Rumpf += "    add r11.xy, r11, -c31.y\n";
				if (k.Modus == 15)
					Rumpf += "    mul r11.xy, r11, c30.x\n";   // c30.x = 24

				Rumpf += "    mad r0.xy, r11, c31.z, r0\n"
				         "    dp3 r0.w, r0, r0\n"
				         "    max r0.w, r0.w, c31.x\n"
				         "    rsq r0.w, r0.w\n"
				         "    mul r0.xyz, r0, r0.w\n";
		...
		if ((k.Modus >= 12 || k.Modus == 5) && k.Hemi)
		{
		...
		for (unsigned i = 0; i < k.Lichter; ++i)
		{
			...
			if (k.Modus == 5 || k.Modus >= 14)
			{
				sprintf_s(z, sizeof(z),
					...
					"    mul r8.xyz, r8, r8.w\n"
					"    dp3_sat r8.w, r8, r0\n"
					// *** DIE BASIS VON pow DARF NIE 0 SEIN (F11). ***
					// pow rechnet exp2(y * log2(x)); log2(0) ist minus
					// unendlich, das Ergebnis NaN. NaN frisst sich durch
					// r9 bis in oC0, und die Flaeche wird schwarz.
					// N.H ist bei jedem vom Licht abgewandten Bildpunkt
					// genau 0 -- der Fall ist also der Normalfall, nicht
					// die Ausnahme.
					"    max r8.w, r8.w, c31.x\n"
					"    pow r8.w, r8.w, c30.x\n"
					"    mul r8.w, r8.w, r5.y\n"
	...
	if ((k.Modus == 5 || k.Modus >= 14) && k.Lichter == 0 && k.Hemi)
		Rumpf += "    add_sat r3.xyz, r3, r9\n";

	if ((k.Modus == 5 || k.Modus >= 14) && k.Lichter > 0)
	{
		// Fresnel nach Schlick: F = F0 + (1 - F0) * (1 - N.V)^5.
		// *** ALS lrp GESCHRIEBEN, NICHT ALS mad. *** Eine Instruktion
		// darf nur EIN Konstantenregister lesen; "mad r, (1-c30.w),
		// r, c30.w" waere zwar kuerzer gedacht, braeuchte aber einen
		// zweiten Konstantenslot fuer (1-F0).
		//   lrp d, s0, s1, s2  =  s2 + s0*(s1-s2)
		//                      =  F0 + X*(1-F0)     mit s1 = 1.0
		//
		// *** DAS ZIEL VON lrp DARF NICHT DIE ERSTE ODER DRITTE QUELLE
		// SEIN (error X5766). *** Deshalb geht das Ergebnis nach r12
		// und nicht zurueck nach r11. r10 waere frei gewesen, ist aber
		// bei BLENDTEXTUREALPHA belegt.
		// Auch hier: 1 - N.V wird genau 0, sobald man frontal auf eine
		// Flaeche sieht -- bei grossen Gebaeudewaenden also staendig.
		// Ohne die Klemmung liefert pow dort NaN (F11).
