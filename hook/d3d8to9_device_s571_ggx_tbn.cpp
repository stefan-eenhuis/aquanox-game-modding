...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetTexture(DWORD Stage, IDirect3DBaseTexture8 *pTexture)
{
	...
	case D3DRTYPE_TEXTURE:
	{
		...
		if (AqNKStufe >= 2 && Stage == 0)
		{
			...
			ProxyInterface->SetTexture(8, T8->AqNormal);
			AqNormalAktiv = (T8->AqNormal != nullptr);
			if (AqNormalAktiv)
				++AqNormalGebunden;

			ProxyInterface->SetTexture(9, T8->AqOrm);
...
bool Direct3DDevice8::AqBaueSchluessel(AqSchluessel &k) const
{
	...
	// *** NUR IN DER ps_3_0-FAMILIE (Modus 5/14/15). *** Der TBN-Block
	...
	k.NormalKarte = (AqNormalAktiv && (AqModus == 5 || AqModus >= 14)) ? 1 : 0;
	k.OrmKarte    = (AqOrmAktiv    && (AqModus == 5 || AqModus >= 14)) ? 1 : 0;
	...
	else
	{
		...
		if (k.NormalKarte)
		{
			...
			if (k.Modus != 14)
			{
				...
				// *** MODUS 15: STOERUNG 24-FACH. *** Unuebersehbar,
				// physikalisch unsinnig -- reine Sichtbarkeitsprobe.
				if (k.Modus == 15)
