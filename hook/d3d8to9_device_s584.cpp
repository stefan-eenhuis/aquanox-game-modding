...
void Direct3DDevice8::AqVorZeichnen()
{
	...
	const bool Prelit =
		...
		&& AqZAn
		&& AqCull != D3DCULL_NONE
		&& !(AqBlendAn != 0 && AqSrcBlend == D3DBLEND_SRCALPHA)
		&& !AqVSLicht && CurrentVertexShaderHandle != 0 && CurrentPixelShaderHandle == 0
	...
	{
		AqSchluessel kn;
		if (!AqBaueSchluessel(kn) || kn.Effekt)
		{
		...
		// Ebenso Modus 20 (Metallprobe, 584): auch Prelit-Waende
		// sollen ihren Metallwert zeigen.
		kn.Modus = static_cast<unsigned char>(
			(AqModus == 18 || AqModus == 20) ? AqModus : 19);
		kn.Hemi = 0;
		kn.Lichter = 0;
		kn.Additiv = 0;
		kn.Glanz = 0;
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
			...
			if (k.Modus == 5 || k.Modus >= 14)
			{
				if (k.OrmKarte)
					Rumpf += "    texld r10, t0, s9\n"
					         // *** r14.y RETTET DEN METALLWERT (584). ***
		...
		if (k.Modus == 19)
		{
			...
			// wuerde sie faelschlich quadrieren.
		}
		else if (k.Additiv)
	...
	// *** MODUS 20: DER METALLWERT ALS GRAUSTUFE (584). *** Weiss =
	// Metall 1, Schwarz = kein Metall -- die Extremprobe nach dem
	// Muster von 14/16/18. Objekte OHNE ORM-Karte rendern normal:
	// was im Probelauf "normal" aussieht, hat schlicht keine Karte.
	if (k.Modus == 20 && k.OrmKarte)
		Rumpf += "    mov r3.xyz, r14.y\n";
