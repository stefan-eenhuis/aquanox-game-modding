...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
			...
			|| ((k.Modus == 18 || k.Modus == 20) && k.Lichter == 0))
		{
			...
			// *** WANDLICHT (593): RELIEF AUF PRELIT-FLAECHEN. *** Das
			// Licht dieser Flaechen ist eingebacken -- die Normalenkarte
			// hatte dort NICHTS, worauf sie antworten konnte (Stefans
			// Befund: "fast alle Flaechen"). Abhilfe: die eingebackene
			// Farbe wird um den RELIEF-UNTERSCHIED zur Blickrichtung
			// moduliert, 1 + c2.w * ((N'.V) - (N.V)) -- eine Art
			// Taucherlampe, die NUR auf die Stoerung reagiert: wo die
			// Karte flach ist, ist der Faktor exakt 1 und das Bild
			// bitgenau wie bisher (Lightmaps, Kaustik-Passes bleiben).
			// c2.w = AQUANOX_WANDLICHT (Vorgabe 2).
