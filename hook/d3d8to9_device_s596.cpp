...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetVertexShaderConstant(DWORD Register, const void *pConstantData, DWORD ConstantCount)
{
	...
	// WVP-Matrix der Prelit-Shader (596): vs c0..c3 mitschreiben. Die
	// Matrix wechselt je Objekt -- das Schmutzig-Flag sorgt dafuer,
	// dass der Prelit-Pfad seine errechnete Kamera je Objekt neu laedt.
	if (AqModus != 0 && pConstantData != nullptr && Register < 4)
	{
		const DWORD Anzahl = (ConstantCount < 4 - Register) ? ConstantCount : (4 - Register);
		memcpy(&AqMatrixVS[Register * 4], pConstantData, Anzahl * 4 * sizeof(float));
...
void Direct3DDevice8::AqVorZeichnen()
{
	...
	if (Prelit && AqModus != 1)
	{
		...
		if (AqKonstSchmutzig)
		{
			...
			// Prelit-Kamera aus der WVP-Matrix (596): das vs-c2 des
			// Licht-Pfads existiert fuer diese Shader nicht.
			if (AqMatrixSchmutzig)
			{
				AqPrelitKameraGueltig = AqKameraAusMatrix(AqMatrixVS, AqPrelitKamera);
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
		...
		if (k.NormalKarte)
		{
			...
			// Normalfall, t1 bei lightmap-zuerst-Zuegen.
	...
	if (k.Modus == 19 && k.NormalKarte)
		Rumpf += "    dp3 r20.w, r0, r7\n"
		         "    dp3 r19.w, r20, r7\n"
		         "    add r20.w, r20.w, -r19.w\n"
		         "    mad r20.w, r20.w, c2.w, c1.y\n"
		         "    max r20.w, r20.w, c31.x\n"
		         "    mul r3.xyz, r3, r20.w\n";
