...
void Direct3DDevice8::AqVorZeichnen()
{
	...
	if (Prelit && AqModus != 1)
	{
		...
		// Ebenso Modus 20 (Metallprobe, 584) und 21 (Wandlicht-
		// Faktorprobe, 598): auch Prelit-Waende sollen zeigen.
		kn.Modus = static_cast<unsigned char>(
			(AqModus == 18 || AqModus == 20 || AqModus == 21) ? AqModus : 19);
		kn.Hemi = 0;
		kn.Lichter = 0;
		kn.Additiv = 0;
		kn.Glanz = 0;
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
		...
		if (k.NormalKarte)
		{
			...
			if (k.Modus != 14)
			{
				...
				// Auch fuer die Faktorprobe Modus 21 (598).
				if (k.Modus == 19 || k.Modus == 21)
	...
	// Null und malte harte schwarze Flecken, die gegen die pulsende
	// Original-Kaustik "flackerten" (Stefans Befund).
