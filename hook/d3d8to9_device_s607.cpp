...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetTexture(DWORD Stage, IDirect3DBaseTexture8 *pTexture)
{
	...
	// macht.
	if (Stage == 0 && !IsRecordingState && AqErsatzAn
		&& pTexture != nullptr && pTexture->GetType() == D3DRTYPE_TEXTURE)
	{
			...
			&& D3DXCreateTextureFromFileA != nullptr)
		{
			...
			if (FAILED(D3DXCreateTextureFromFileA(ProxyInterface, PfadE, &TE->AqErsatz)))
				TE->AqErsatz = nullptr;
		}
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
		...
		if (k.Modus >= 10 || k.Modus == 5 || k.Modus >= 14)
		{
			...
			// UV-Kachelung (607): r21 = vervielfachte Objekt-UVs der
			// Karten-Stufe; c5.x = AQUANOX_KACHEL. Alle Objekttextur-
			// und Karten-Abtastungen nutzen dann r21.
			if (AqKachelAn)
			{
				sprintf_s(z, sizeof(z), "    mul r21, t%u, c5.x\n",
					static_cast<unsigned>(k.KartenStufe));
				Rumpf += z;
			}
