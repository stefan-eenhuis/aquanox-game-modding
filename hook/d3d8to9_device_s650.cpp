...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
		...
		if (k.Modus >= 10 || k.Modus == 5 || k.Modus >= 14)
		{
			...
			// *** EIGENES PUNKTLICHT (649): NUR PRELIT-BASIS (19). ***
			// c6 = Weltposition + 1/R^2 (Statics: Objektraum = Welt,
			// 607b), c7 = Farbe*Intensitaet -- Puls/Blinken rechnet
			// die CPU (AqLichtTick). N.L auf der GESTOERTEN Normale
			// r0 -> das Relief antwortet; der Detailpass des selben
			// Objekts formt das Licht dann mit (645er-Mechanik).
			// Additiv auf r2 (Prelit-Farbe), VOR der Texturkette.
			// Falloff (1-d^2/R^2)^2: weich, null ausserhalb R.
