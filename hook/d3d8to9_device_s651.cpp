...
// 649: Puls/Blinken je Frame. Version 1 nutzt das ERSTE Licht der
// Datei (ein Shader-Slot c6/c7); weitere Lichter sind geladen und
// warten auf den Mehrslot-Ausbau. Die Zeitfunktion liegt bewusst
// auf der CPU: der Shader bleibt frei von Zeitkonstanten, und
// jede kuenftige Kurve (Flackern, Rampen) kostet nur C++.
void Direct3DDevice8::AqLichtTick()
{
	if (!AqLichterAn)
		return;
	const AqLicht &l = AqLichter[0];
	float f = 1.0f;
	const DWORD t = GetTickCount();
	if (l.Modus == 1 && l.PeriodeMs > 1.0f)
		f = 0.55f + 0.45f * sinf(static_cast<float>(t)
			* (6.2831853f / l.PeriodeMs));
	else if (l.Modus == 2 && l.PeriodeMs > 1.0f)
		f = (static_cast<float>(t) - l.PeriodeMs
			* floorf(static_cast<float>(t) / l.PeriodeMs))
			< l.PeriodeMs * 0.5f ? 1.0f : 0.0f;
	const float r2 = l.Radius > 1.0f ? l.Radius * l.Radius : 1.0f;
	AqLichtPos[0] = l.Pos[0];
	AqLichtPos[1] = l.Pos[1];
	AqLichtPos[2] = l.Pos[2];
	AqLichtPos[3] = 1.0f / r2;
	AqLichtWert[0] = l.Farbe[0] * l.Intensitaet * f;
	AqLichtWert[1] = l.Farbe[1] * l.Intensitaet * f;
	AqLichtWert[2] = l.Farbe[2] * l.Intensitaet * f;
	AqLichtWert[3] = 0.0f;
	AqKonstSchmutzig = true;
}
	...
	if (Prelit && AqModus != 1)
	{
		...
		if (AqKonstSchmutzig)
		{
			...
			ProxyInterface->SetPixelShaderConstantF(2, AqRelief, 1);
			{
				...
				if (AqLichterAn)
				{
					// 649: eigenes Punktlicht (c6/c7)
					ProxyInterface->SetPixelShaderConstantF(6, AqLichtPos, 1);
					ProxyInterface->SetPixelShaderConstantF(7, AqLichtWert, 1);
				}
			}
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	// *** EIGENES PUNKTLICHT (649, Standort-Fix 651): PRELIT-BASIS. ***
	// c6 = Weltposition + 1/R^2 (Statics: Objektraum = Welt, 607b),
	// c7 = Farbe*Intensitaet -- Puls/Blinken rechnet die CPU
	// (AqLichtTick). N.L auf der GESTOERTEN Normale r0 (Relief
	// antwortet); Falloff (1-d^2/R^2)^2. ADDITIV auf r3 am KETTEN-
	// ENDE -- der erste Standort auf r2 war die 594b-Falle (die
	// Basis-Ketten lesen DIFFUSE nicht). NACH dem Wandlicht: der
	// Praegefaktor soll den Lichtschein nicht mitmodulieren; der
	// Detailpass desselben Objekts formt ihn ohnehin mit (645).
	if (k.Modus == 19 && AqLichterAn)
		Rumpf += "    add r22.xyz, c6, -t6\n"
		         "    dp3 r22.w, r22, r22\n"
		         "    mul r23.w, r22.w, c6.w\n"
		         "    add r23.w, c31.w, -r23.w\n"
