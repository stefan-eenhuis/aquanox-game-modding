	...
	if (Prelit && AqModus != 1)
	{
		...
		if (AqKonstSchmutzig)
		{
			...
			ProxyInterface->SetPixelShaderConstantF(2, AqRelief, 1);
			{
				...
				// 652: eigene Punktlichter, je Licht 2 Register ab
				// c200 (kollisionsfreier Bereich, nur Modus 19 liest).
				if (AqLichterAn)
					ProxyInterface->SetPixelShaderConstantF(200,
						&AqLichtDaten[0][0], 2 * AqLichterZahl);
			}
	...
	if (Detail && AqModus != 1)
	{
		...
		if (AqKonstSchmutzig)
		{
			...
			ProxyInterface->SetPixelShaderConstantF(2, AqRelief, 1);
			{
				...
				// 653: Licht-Upload AUCH hier -- der 23er traegt die
				// Lichtbloecke jetzt selbst (Terrain-Abdeckung).
				if (AqLichterAn)
					ProxyInterface->SetPixelShaderConstantF(200,
						&AqLichtDaten[0][0], 2 * AqLichterZahl);
			}
