...
void Direct3DDevice8::AqInit(DWORD BehaviorFlags)
{
	...
	// AQUANOX_TERRAINMIX=1 (661): Der Terrain-Detailzug zeigt im
	...
	// St1) und material_1 (Fels, St3) mit; der Shader mischt sie
	// per Neigungsmaske dazu (flach Sand, steil Fels -- so ist die
	// Selektorkarte gemalt, Korrelation +0.61 zur Steilheit).
	char TM[8] = {};
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	const bool TerrainMix = AqTerrainMixAn && k.Modus == 23
	...
	if ((k.Modus >= 5 && k.Modus <= 9) || k.Modus >= 14)
	{
		...
		if (TerrainMix)
			Kopf += "    def c6, 0.0727273, 0.25, 0.0, 0.0\n"
			        "    dcl_2d s1\n"
			        "    dcl_2d s3\n";
