...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetVertexShaderConstant(DWORD Register, const void *pConstantData, DWORD ConstantCount)
{
	...
	if (AqModus != 0 && pConstantData != nullptr && Register <= 15 && Register + ConstantCount > 15)
	{
	...
	{
		memcpy(AqVSC16, static_cast<const float *>(pConstantData) + (16 - Register) * 4,
			4 * sizeof(float));
		AqVSC16Gueltig = true;
	}
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	for (unsigned s = 0; s < k.Stufen; ++s)
	{
		...
		// UNDOKUMENTIERTE Ops (MODULATEALPHA_ADDCOLOR, SELECTARG
		// jenseits Stufe 0): Gate NICHT anwenden (konservativ, Zug
		// bleibt unveraendert) + Einmal-Logzeile KAUSTIK-OP-UNBEKANNT.
