...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetVertexShaderConstant(DWORD Register, const void *pConstantData, DWORD ConstantCount)
{
	...
	// 671: c15/c16 spiegeln (Muster wie c2 oben). Dort laedt die
	// Engine die Parameter ihrer Terrain-Selektor-UV-Formel
	// (oT1 = (v0.xy + c15) * c16) hoch; der 23er-Bindungspfad reicht
	// beide als ps c7 an die TERRAINMIX=2-Vollvariante weiter. Der
	// 668er-Kamera-Differenz-Weg ist tot: der Messlauf 671 bewies
	// WeltKam=0 -- die Engine setzt SetTransform(VIEW) NIE.
	// *** SELEKTOR-SIGNATUR-FILTER (673). *** Der 00:34-Messlauf
	// zeigte TERRAINMIX2: c15=(-220,-110) c16=(0.00909,0.00909)
	// frisch=11 -- der Spiegel hatte die MATERIAL-Kachelwerte
	// erwischt (0.00909 = 1/110 = W0-Skala aus material.des), NICHT
	// die Selektor-Formel (1/4096 = 0.000244...): die Engine
	// ueberschreibt c15/c16 zwischen Selektor- und Detail-Upload.
	// Folge im Bild: Fels/Sand-Maske 37x zu fein gekachelt und
	// springend beim Kachelwechsel (Stefans Terrain-Ruckeln).
	// Deshalb NUR uebernehmen, wenn das FRISCH geschriebene c16 zur
	// Selektorskala passt (0 < c16.xy < 0.002; die Materialskalen
	// liegen bei >= 1/256 = 0.0039 -- Faktor >30 Abstand, die
	// Schwelle trennt sauber). c15 haengt am SELBEN Aufruf: die
	// Formel ist ein PAAR -- ein Material-c15 zu einem Selektor-c16
	// waere exakt der alte Fehler. Frische-Flags bleiben (jetzt:
	// "je einen PASSENDEN Upload gesehen"). Meldet die TERRAINMIX2-
	// Zeile kuenftig frisch=00/sig=0, laedt die Engine die
	// Selektorwerte getrennt oder nie -- dann Koppelung neu pruefen.
	if (AqModus != 0 && pConstantData != nullptr && Register <= 16 && Register + ConstantCount > 16)
	{
		const float *const C16 = static_cast<const float *>(pConstantData) + (16 - Register) * 4;
		if (C16[0] > 0.0f && C16[0] < 0.002f && C16[1] > 0.0f && C16[1] < 0.002f)
		{
			memcpy(AqVSC16, C16, 4 * sizeof(float));
			AqVSC16Gueltig = true;
			if (Register <= 15)
			{
				memcpy(AqVSC15, static_cast<const float *>(pConstantData) + (15 - Register) * 4,
					4 * sizeof(float));
				AqVSC15Gueltig = true;
			}
		}
...
void Direct3DDevice8::AqVorZeichnen()
{
	...
	if (Detail && AqModus != 1)
	{
		...
		// 671: Vollvariante des Terrainmix -- c7 = GESPIEGELTE
		// Engine-Konstanten (c15.xy, c16.xy) fuer die Selektor-UV
		// uv = (t6.xy + c7.xy) * c7.zw. Der 668er-Kamera-Differenz-
		// Weg samt 670er-4-WE-Rasterung ist AUSGEBAUT: der 23:49-
		// Messlauf bewies WeltKam=0 -- die Engine setzt
		// SetTransform(VIEW) NIE, der Kamera-Weg war tot (c7 blieb
		// dauerhaft auf dem 0-Rueckfall). Upload BEWUSST vor JEDEM
		// Detailzug statt im Schmutzig-Block (668/4-Lehre: EIN FLAG
		// MIT MEHREREN VERBRAUCHERN IST KEIN JE-ZUG-EREIGNIS) --
		// die Engine laedt c15/c16 je Kachel neu, der Schatten ist
		// beim Detailzug frisch. Kosten: 1 Register je Terrain-Zug.
		if (AqTerrainMix == 2)
		{
			// Rueckfall ohne je gesehene c15/c16 (Frische-Flags):
			// Schatten sind 0-initialisiert -> uv konstant 0, der
			// Selektor liefert EIN Texel -- sichtbar einfarbig statt
			// still falsch verschoben; die Logzeile unten macht den
			// Fall sofort diagnostizierbar.
			const float V7[4] = { AqVSC15[0], AqVSC15[1],
				AqVSC16[0], AqVSC16[1] };
			ProxyInterface->SetPixelShaderConstantF(7, V7, 1);
#ifndef D3D8TO9NOLOG
			// 671: Einmal beim ERSTEN ECHTEN Terrainzug (gleiche
			// Signatur wie der TerrainMix-Block des Generators --
			// Stationen laufen auch durch Modus 23) die gespiegelten
			// Werte festhalten. Plausibilitaet: c16 ist die Selektor-
			// Skala (Groessenordnung 1/4096 auf 1h1?) -- die Werte
			// werden einfach ausgegeben und im Log beurteilt.
			if (!AqTerrainMix2Gemeldet
				&& kd.Stufen == 1 && kd.NormalKarte != 0
				&& kd.Tex[0] == 1
				&& kd.Op[0][0] == D3DTOP_SELECTARG2)
			{
				// 673: sig=1 heisst, der Schatten traegt Werte der
				// SELEKTORSKALA (der Signatur-Filter im Hook laesst
				// nur solche durch); sig=0 heisst, es kam nie ein
				// passender Upload -- c7 steht auf dem 0-Rueckfall
				// (Selektor einfarbig, EIN Texel).
				const bool Sig = AqVSC16[0] > 0.0f && AqVSC16[0] < 0.002f
					&& AqVSC16[1] > 0.0f && AqVSC16[1] < 0.002f;
				AqTerrainMix2Gemeldet = true;
				LOG << "> AquaNox-TERRAINMIX2: c15="
				    << AqVSC15[0] << "," << AqVSC15[1]
				    << " c16=" << AqVSC16[0] << "," << AqVSC16[1]
				    << " frisch=" << (AqVSC15Gueltig ? 1 : 0)
				    << (AqVSC16Gueltig ? 1 : 0)
				    << " sig=" << (Sig ? 1 : 0) << std::endl;
			}
#endif
		}
		return;
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	// gebunden, aber ketten-inaktiv. Der Detail-VS
	// (rep_terrainprelit_vs_dmfm) schreibt als einzige UV
	// oT0 = (v0.xy + c19) * c20; die Selektor-UV der Familie
	// (oT1 = (v0.xy + c15) * c16) rechnet er nicht selbst.
	// *** SEIT 671: c15/c16-SPIEGELUNG STATT WELTREKONSTRUKTION. ***
	// Der 668er-Weg (welt = t6 - c7 mit c7 = PrelitKamera -
	// WeltKamera) ist TOT: der 23:49-Messlauf bewies WeltKam=0 --
	// die Engine setzt SetTransform(VIEW) NIE, c7 blieb auf dem
	// 0-Rueckfall. Stattdessen jetzt EXAKT die Engine-Formel: der
	// SetVertexShaderConstant-Hook schattet c15/c16 (die Engine
	// laedt sie fuer die Shader-Familie hoch), der Bindungspfad
	// reicht sie als EIN ps-Register weiter (c7 = c15.x, c15.y,
	// c16.x, c16.y), und der Shader rechnet
	// uv = (t6.xy + c7.xy) * c7.zw (t6 = v0 durchgereicht).
	// KEIN V-Flip (668-Befund: Selektortextur behaelt die TGA-
	// Dateizeilenfolge, die Engine sampelt flip-frei). X5584: c7
	// je Anweisung einmal (zwei Anweisungen).
	...
	if (TerrainMix)
	{
		...
		Rumpf += z;
		if (AqTerrainMix == 2)
			// Vollvariante (671): Fels-Gewicht aus der Selektorkarte,
			// UV nach der GESPIEGELTEN Engine-Formel
			// uv = (t6.xy + c7.xy) * c7.zw (Herleitung im Block-
			// kommentar oben; c7.zwzw: z->x, w->y fuer die .xy-Maske).
			// r26 wird gleich vom Misch-Schwanz wiederverwendet --
			// das Alpha ist dann laengst in r23.w gerettet.
			Rumpf += "    add r26.xy, t6, c7\n"
			         "    mul r26.xy, r26, c7.zwzw\n"
			         "    texld r26, r26, s2\n"
			         "    mov r23.w, r26.w\n";
		else
	...
	{
		// *** SPRUNGKOMPENSIERTE BASIS AUF DEM TERRAIN (673). ***
		// Die rohen t6-Schraegsummen springen auf dem Terrain bei
		// der Sektor-Neuverankerung (t6 = v0 durchgereicht; die
		// Engine verankert die Kacheln neu und schiebt den Versatz
		// nach c15) -- das Anti-Kachel-Muster ruckte im Sektortakt
		// mit (Stefans Beleuchtungs-Ruckeln beim Fliegen). In der
		// TerrainMix-VOLLVARIANTE liegt c7 = (c15.xy, c16.xy) im
		// Shader: (t6.xy + c7.xy) ist die sprungfreie Basis der
		// Engine-Selektorformel -- die xy-Kompensation absorbiert
		// den Sektorsprung exakt. z bleibt ROH: die Hoehe wird bei
		// der Neuverankerung vermutlich nicht bzw. nur minimal
		// versetzt (unbewiesen; spraenge sie doch, bliebe ein
		// Rest-Ruckeln sichtbar -- dann z ebenfalls kompensieren).
		// X5584: c7 und c5 je in eigener Anweisung. Nicht-Terrain-
		// 23er (kein TerrainMix-Schluessel, kein c7 im Shader):
		// roher Weg unveraendert -- Stationsurspruenge sind stabil.
		if (TerrainMix && AqTerrainMix == 2)
			Rumpf += "    add r24.xy, t6, c7\n"
	...
	if ((k.Modus >= 5 && k.Modus <= 9) || k.Modus >= 14)
	{
		...
		// c6.z (1/4096) ist seit 671 UNBENUTZT -- die Vollvariante
		// rechnet mit den gespiegelten c15/c16 (ps c7), nicht mehr
		// mit der Weltrekonstruktion; der def bleibt als Platzhalter.
		if (TerrainMix)
		{
			Kopf += "    def c6, 0.0727273, 0.25, 0.000244140625, 0.0\n"
			        "    dcl_2d s1\n"
			        "    dcl_2d s3\n";
			// Vollvariante (668): der Fels-Selektor liegt auf s2
