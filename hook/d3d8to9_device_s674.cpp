...
// 649/652: Puls/Blinken je Frame fuer ALLE Lichter. Die Zeit-
// funktion liegt bewusst auf der CPU: der Shader bleibt frei von
// Zeitkonstanten, jede kuenftige Kurve (Flackern, Rampen) kostet
// nur C++. Der Phasenversatz (i*357 ms) entzerrt die Pulse --
// sechzehn synchron atmende Staebe saehen nach Discoanlage aus.
void Direct3DDevice8::AqLichtTick()
{
	if (!AqLichterAn)
		return;
	const DWORD t = GetTickCount();
	for (int i = 0; i < AqLichterZahl && i < 16; ++i)
	{
		const AqLicht &l = AqLichter[i];
		const float tv = static_cast<float>(t) + i * 357.0f;
		float f = 1.0f;
		if (l.Modus == 1 && l.PeriodeMs > 1.0f)
			f = 0.55f + 0.45f * sinf(tv * (6.2831853f / l.PeriodeMs));
		else if (l.Modus == 2 && l.PeriodeMs > 1.0f)
			f = (tv - l.PeriodeMs * floorf(tv / l.PeriodeMs))
				< l.PeriodeMs * 0.5f ? 1.0f : 0.0f;
		const float r2 = l.Radius > 1.0f ? l.Radius * l.Radius : 1.0f;
		float *const p = AqLichtDaten[i * 2];
		float *const w = AqLichtDaten[i * 2 + 1];
		p[0] = l.Pos[0];
		p[1] = l.Pos[1];
		p[2] = l.Pos[2];
		p[3] = 1.0f / r2;
		w[0] = l.Farbe[0] * l.Intensitaet * f;
		w[1] = l.Farbe[1] * l.Intensitaet * f;
		w[2] = l.Farbe[2] * l.Intensitaet * f;
		w[3] = 0.0f;
	}
	AqKonstSchmutzig = true;
}
// 654: Lichtdaten fuer den AKTUELLEN Zug hochladen. Die Welt-
// positionen wandern in den Objektraum des Zugs: Licht_obj =
// PrelitKamera + (Licht_welt - Auge_welt) -- exakt fuer nur
// VERSCHOBENE Statics (Terrain-Sektoren, Stationsmodule); fuer
// W=I-Zuege degeneriert die Formel zu Licht_welt (PrelitKamera ==
// Auge). Rotierte Objekte bekommen eine falsche RICHTUNG des
// Versatzes -- bewusst in Kauf genommen (Statics rotieren selten).
// Ohne VIEW-Matrix der Engine bleibt die rohe Weltposition
// (Fallback = Verhalten 653).
void Direct3DDevice8::AqLichtUpload()
{
	if (!AqLichterAn)
		return;
	float LD[32][4];
	const int n = AqLichterZahl < 16 ? AqLichterZahl : 16;
	memcpy(LD, AqLichtDaten, 2 * n * 4 * sizeof(float));
	if (AqWeltKameraGueltig && AqPrelitKameraGueltig)
	{
		const float dx = AqPrelitKamera[0] - AqWeltKamera[0];
		const float dy = AqPrelitKamera[1] - AqWeltKamera[1];
		const float dz = AqPrelitKamera[2] - AqWeltKamera[2];
		for (int i = 0; i < n; ++i)
		{
			LD[2 * i][0] += dx;
			LD[2 * i][1] += dy;
	...
	ProxyInterface->SetPixelShaderConstantF(200, &LD[0][0], 2 * n);
}
HRESULT STDMETHODCALLTYPE Direct3DDevice8::Present(const RECT *pSourceRect, const RECT *pDestRect, HWND hDestWindowOverride, const RGNDATA *pDirtyRegion)
{
	...
	AqLichtTick();   // 649: Puls/Blinken der eigenen Lichter
...
{
	// 654: WELT-KAMERAPOSITION aus der VIEW-Matrix. Die Engine muss
	// sie fuer ihre Fixed-Function-Zuege setzen; Auge = -t * R^T.
	// Gebraucht von den eigenen Lichtern: Licht_obj = PrelitKamera
	// + (Licht_welt - Auge_welt) -- korrekt fuer alle NUR
	// VERSCHOBENEN Statics (Terrain-Sektoren, Stationsmodule),
	// deren lokale Urspruenge das Licht sonst "zufaellig"
	// verstreuten (Stefans 653-Befund).
	// 670: IMMER erfassen, nicht nur bei eigenen Lichtern -- auch der
	// 668er-Terrainmix (c7 = PrelitKamera - WeltKamera) braucht die
	// Weltkamera; das fruehere "&& AqLichterAn" liess c7 ohne
	// Lichtdatei dauerhaft auf dem 0-Rueckfall (669er-Log: die
	// WELTKAMERA-Zeile fehlte komplett).
	if (State == D3DTS_VIEW && pMatrix != nullptr)
	{
		const float *m = &pMatrix->_11;
		AqWeltKamera[0] = -(m[12] * m[0] + m[13] * m[1] + m[14] * m[2]);
		AqWeltKamera[1] = -(m[12] * m[4] + m[13] * m[5] + m[14] * m[6]);
		AqWeltKamera[2] = -(m[12] * m[8] + m[13] * m[9] + m[14] * m[10]);
		AqWeltKameraGueltig = true;
#ifndef D3D8TO9NOLOG
		if (!AqWeltKameraGemeldet)
		{
			AqWeltKameraGemeldet = true;
			LOG << "> AquaNox-WELTKAMERA: " << AqWeltKamera[0] << " "
			    << AqWeltKamera[1] << " " << AqWeltKamera[2] << std::endl;
		}
#endif
	}
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetTexture(DWORD Stage, IDirect3DBaseTexture8 *pTexture)
{
	if (pTexture == nullptr)
	{
		...
		if (SUCCEEDED(hrLoesen) && Stage < 8 && !IsRecordingState)
		{
			...
			if (Stage == 1)
				AqStufe1Tex = nullptr;
			if (Stage == 2)
				AqHash2 = 0;   // 669: Stufe-2-Schatten (Schiffs-Kaustik)
			if (Stage == 3)
				AqHash3 = 0;   // 670: Stufe-3-Schatten (Kaustik-Diagnose)
			if (Stage == 0)
			{
				AqNormalAktiv = false;
				AqHash0 = 0;
	...
	case D3DRTYPE_TEXTURE:
	{
		...
		if (Stage == 1 && !IsRecordingState)
			AqStufe1Tex = T8;
		// Stufe-2-Hash (669): die Schiffs-Kaustik haengt als
		// BLENDTEXTUREALPHA-Stufe 2 im Modus-5-Hauptzug -- der
		// Schluesselbau erkennt sie am Frame-Hash, analog AqHash0.
		if (Stage == 2 && !IsRecordingState)
			AqHash2 = T8->AqHash;
		// Stufe-3-Hash (670): fuer die KAUSTIK-ZUG-Diagnose -- die
		// Terrain-Kette traegt material_1 auf St3, die Kaustik koennte
		// dort ebenso sitzen wie auf St2.
		if (Stage == 3 && !IsRecordingState)
			AqHash3 = T8->AqHash;
#ifndef D3D8TO9NOLOG
		// 670: KAUSTIK-TRAEGER-DIAGNOSE. Das TEXTUR-Log oben meldet je
		// Hash nur die ERSTE Bindung -- ein Kaustik-Frame, der zuerst
		// auf St0 und spaeter auf St2 erscheint, blieb damit auf St2
		// unsichtbar (669: weder KAUSTIK-23 noch KAUSTIK-S2 feuerte,
		// obwohl die Kaustik sichtbar lief). Fuer die 32 Kaustik-Hashes
		// deshalb EINMAL JE (Hash,Stufe)-KOMBINATION melden; Salz
		// Hash^Stage^0xB1D im AqTexGesehen-Muster.
		if (T8->AqHash != 0 && AqKaustikHashes.count(T8->AqHash) != 0
			&& AqTexGesehen.insert(T8->AqHash ^ Stage ^ 0xB1DULL).second)
			LOG << "> AquaNox-KAUSTIK-BIND: " << std::hex << T8->AqHash
			    << std::dec << " St=" << Stage << std::endl;
#endif
#ifndef D3D8TO9NOLOG
		if (T8->AqHash != 0 && AqTexGesehen.insert(T8->AqHash).second)
		{
			// 661: Stage der ERSTBINDUNG mitloggen -- beweist die
			// Stufenbelegung der Terrain-Kette (material_1 nur St3).
			LOG << "> AquaNox-TEXTUR: " << std::hex << T8->AqHash << std::dec
				<< "  " << T8->AqBreite << "x" << T8->AqHoehe
				<< " St=" << Stage << std::endl;
	...
	case D3DRTYPE_VOLUMETEXTURE:
		...
		if (Stage == 2 && !IsRecordingState) AqHash2 = 0;
		if (Stage == 3 && !IsRecordingState) AqHash3 = 0;   // 670
...
// Kaustik-Unterdrueckung (644, verschaerft 650, repariert 669): der
// Zug faellt ganz aus, wenn seine Stufe-0-Textur ein Kaustik-Frame
// ist UND er wie ein Kaustik-Zug mischt. Die 650er-Verschaerfung
// auf ADDITIV (Dest=ONE) war richtig gegen die Schiffs-Regression,
// machte AQUANOX_KAUSTIK=0 aber in Wahrheit WIRKUNGSLOS: die echten
// Statics-Kaustik-Zuege laufen als eigener 2x-Multiplikationszug
// (Src=DESTCOLOR/Dest=SRCCOLOR, 669-Diagnose) -- Dest=ONE traf sie
// nie. Jetzt gelten BEIDE Formen (additiv UND 2x-Multiplikation);
// SRCALPHA-Zuege (die Schiffs-BASIS, 650-Forensik) bleiben nach wie
// vor unerreichbar. Die SCHIFFS-Kaustik selbst (Stufe 2 im Modus-5-
// Hauptzug) ist per Skip prinzipiell NICHT abschaltbar -- ein Skip
// fraesse das ganze Schiff; sie laeuft bei AQUANOX_KAUSTIK=0 ueber
// das Shader-Gate mit f=0 (Generator, 669). Jeder Treffer wird
// protokolliert (einmal je Hash), damit diese Fehlerklasse nie
// wieder unsichtbar ist. VOR AqVorZeichnen: kein Zustand angefasst.
bool Direct3DDevice8::AqKaustikZug()
{
	if (!AqKaustikAus || !AqBlendAn)
		return false;
	const bool Additiv = (AqDestBlend == D3DBLEND_ONE);
	const bool Statics = (AqSrcBlend == D3DBLEND_DESTCOLOR
		&& AqDestBlend == D3DBLEND_SRCCOLOR);
	if ((!Additiv && !Statics) || AqKaustikHashes.count(AqHash0) == 0)
		return false;
#ifndef D3D8TO9NOLOG
	if (AqTexGesehen.insert(AqHash0 ^ 0xCA57ULL).second)
		LOG << "> AquaNox-KAUSTIK-SKIP: " << std::hex << AqHash0
		    << std::dec << std::endl;
#endif
	return true;
}
	...
	*pHandle = 0;
	...
	if (pFunction != nullptr)
	{
			...
			&& SourceCode.find("oT7") == std::string::npos)
		{
			...
			// *** 674: SELEKTOR-UV-DURCHREICHUNG FUER DEN TERRAIN-
			// DETAIL-VS (nur TERRAINMIX=2). *** Die statische
			// Herleitung (Handoff 674) bewies: die Selektor-UV der
			// Terrain-Familie ist ein VERTEXDATUM (v3; mmsm:
			// "mov oT0.xy, v3"), kein Konstantenpaar -- und die
			// gemeinsame Deklaration ALLER terrainprelit-Shader
			// (Stack-Aufbau in 0x50b7a0: FLOAT3+FLOAT3+D3DCOLOR+
			// FLOAT2+FLOAT1, 40 Byte) garantiert, dass v3 auch im
			// Vertexstrom des Detailzugs liegt -- der dmfm-VS liest
			// es nur nicht. Erkennung BYTE-EXAKT am Disassemblat:
			// das Zeilenpaar "add r0.xy, v0, c19" + "mul oT0.xy,
			// r0, c20" trifft von allen 79 vs-Blobs der Exe GENAU
			// rep_terrainprelit_vs_dmfm (Scan 674); kein Shader der
			// Exe schreibt selbst oT4 (zusaetzlich abgesichert).
			// v3 ist FLOAT2 -- nur .xy schreiben. AqNachVS3 sieht
			// das v3 und deklariert dcl_normal v3 (Registernummer-
			// Semantik, Tabelle Zeile 1330 ff.); oT4 wird dort zu
			// o7/dcl_texcoord4. Der Generator liest es als t4.
			const bool TerrainDetailVS = AqTerrainMix == 2
				&& SourceCode.find("add r0.xy, v0, c19") != std::string::npos
				&& SourceCode.find("mul oT0.xy, r0, c20") != std::string::npos
				&& SourceCode.find("oT4") == std::string::npos;
			if (TerrainDetailVS)
				AqSelektorDurchgereicht = true;   // Diagnose (Logzeile)

			const std::string Zeilen = std::string(TerrainDetailVS
				? "    mov oT4.xy, v3 /* AquaNox 674: Selektor-UV (Vertexdatum) */\n"
				: "")
				+ "    mov oT5, v1 /* AquaNox: Normale, Objektraum */\n"
				"    mov oT6, v0 /* AquaNox: Position, Objektraum */\n"
				"    mov oT7, v2 /* AquaNox: Vertexfarbe */\n";
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetVertexShaderConstant(DWORD Register, const void *pConstantData, DWORD ConstantCount)
{
	...
	// *** 674: DER c15/c16-SPIEGEL (671) SAMT SIGNATUR-FILTER (673)
	// IST AUSGEBAUT -- ER JAGTE EIN PHANTOM. *** Der 00:52-Messlauf
	// bewies: der 673er-Filter (0 < c16 < 0.002) akzeptierte NIE
	// einen Upload (sig=0, Schatten blieben 0). Die statische
	// Herleitung an den Kachel-Orchestratoren (FUN_004fbfe0/
	// 0x4fd550, Helfer 0x4f2b40 = SetVertexShaderConstant in den
	// Spiegel 0x692420+reg*16, fmod-Helfer 0x4fd340 schreibt direkt
	// den Register-15-Slot 0x692510) zeigte die vollstaendige
	// Formel: c16 = (1/W,1/W,1/W,1/W) mit W = MATERIAL-Fenster aus
	// material.des (1h1: 110/32; Detailpaar c19/c20 analog mit
	// W3=8), c15.xy = -floor(S/W)*W mit S = Weltanker (zwei floats
	// bei Objekt+0x270 des Render-Kontexts). Es gibt KEINEN
	// Selektorskala-Upload: das mmsm-VS-Disassemblat ("mov oT0.xy,
	// v3") beweist, dass die Selektor-UV ein VERTEXDATUM ist --
	// (v0.xy+c15)*c16 ist die MATERIAL-UV (oT1), die 668er-
	// Formelzuordnung war seitenverkehrt. Die gemessenen Werte
	// c15=(-220,-110), c16=1/110 waren also KORREKTE Engine-Werte
	// (4096/110 = 37.2 = exakt Stefans "37x zu fein"). Nachfolger:
	// v3-Durchreichung der dmfm-Zweitfassung (CreateVertexShader)
	// + direkte s2-Abtastung im Generator.
...
HRESULT STDMETHODCALLTYPE Direct3DDevice8::SetStreamSource(UINT StreamNumber, IDirect3DVertexBuffer8 *pStreamData, UINT Stride)
{
	...
	// 674: Schrittweite von Stream 0 schatten -- reine Diagnose fuer
	// die TERRAINMIX2-Logzeile (Soll der Terrain-Familie: 40 Byte;
	// die gemeinsame Deklaration aus 0x50b7a0 ist FLOAT3+FLOAT3+
	// D3DCOLOR+FLOAT2+FLOAT1, v3 = Selektor-UV bei Offset 28).
	if (StreamNumber == 0)
		AqStrom0Schritt = Stride;
...
void Direct3DDevice8::AqInit(DWORD BehaviorFlags)
{
	...
	// KAUSTIK-HASHES (644/669): Die Liste der 32 Kaustik-Frame-Hashes
	// (mod_docu\texturen\kaustik_hashes.txt, Bildabgleich-Inventur)
	// wird seit 669 IMMER geladen -- sie speist nicht mehr nur den
	// Draw-Skip (KAUSTIK=0), sondern auch die Kaustik-ERKENNUNG im
	// Schluesselbau (Statics-St0- und Schiffs-St2-Gate). Ohne Datei
	// bleiben Skip und Gates wirkungslos; die Logzeile meldet das.
	{
		FILE *kdatei = nullptr;
		if (fopen_s(&kdatei, "mod_docu\\texturen\\kaustik_hashes.txt", "r") == 0
			&& kdatei != nullptr)
		{
			char kzeile[64];
			while (fgets(kzeile, sizeof(kzeile), kdatei) != nullptr)
			{
				const unsigned long long kh = _strtoui64(kzeile, nullptr, 16);
				if (kh != 0)
					AqKaustikHashes.insert(kh);
			}
			fclose(kdatei);
		}
	}
	// AQUANOX_KAUSTIK=0 heisst seit 669: Kaustik KOMPLETT aus --
	// Statics-Zuege per Draw-Skip (AqKaustikZug, repariert: auch
	// DESTCOLOR/SRCCOLOR), die Schiffs-Stufe per Shader-Gate f=0.
	char KJ[8] = {};
	if (GetEnvironmentVariableA("AQUANOX_KAUSTIK", KJ, sizeof(KJ)) != 0
		&& KJ[0] == '0')
		AqKaustikAus = !AqKaustikHashes.empty();
	// AQUANOX_KAUSTIK_RICHTUNG=s (669, Stefans 667b-Befund): Kaustik
	// nur auf OBERSEITEN -- f = (1-s) + s*sat(N.up). s=0 (Vorgabe) =
	// bisheriges Verhalten; der Grundanteil (1-s) verhindert voellig
	// tote Waende. Prozesskonstant, geklemmt auf 0..1; der Wert wird
	// als def-Literal (c32) in die betroffenen Shader gebacken.
	char KR[16] = {};
	if (GetEnvironmentVariableA("AQUANOX_KAUSTIK_RICHTUNG", KR, sizeof(KR)) != 0)
	{
		AqKaustikRichtung = static_cast<float>(atof(KR));
		if (AqKaustikRichtung < 0.0f) AqKaustikRichtung = 0.0f;
		if (AqKaustikRichtung > 1.0f) AqKaustikRichtung = 1.0f;
	}
#ifndef D3D8TO9NOLOG
	LOG << "> AquaNox-KAUSTIK: " << (AqKaustikAus ? "AUS" : "an")
	    << ", Richtung s=" << AqKaustikRichtung << ", "
	    << AqKaustikHashes.size() << " Hashes"
	    << (AqKaustikHashes.empty() ? " (DATEI FEHLT/LEER)" : "")
	    << "." << std::endl;
#endif

	// AQUANOX_ANTIKACHEL: Staerke der Grossskala-Selbstmodulation
	// gegen sichtbare Texturwiederholung in der Ferne (649, Stefans
	// Sediment-Befund). c5.z = feste Weltskala (krumm gegen
	// Achsen-Alias), c5.w = Staerke.
	char AK[16] = {};
	if (GetEnvironmentVariableA("AQUANOX_ANTIKACHEL", AK, sizeof(AK)) != 0)
		AqKachel[3] = static_cast<float>(atof(AK));
	AqKachel[2] = 0.0061f;
	AqAntikachelAn = AqKachel[3] > 0.0001f;

	// AQUANOX_TERRAINMIX (661/668): Der Terrain-Detailzug zeigt im
	// Original UEBERALL nur material_3 (Kies, Stufe 0 aktiv) -- die
	// Fels/Sediment-Vielfalt der Selektorkarte erreicht ihn nie
	// (660er-Diagnose). Die Engine BINDET aber material_0 (Sand,
	// St1), material_1 (Fels, St3) und den engine-gebauten Fels-
	// Selektor (St2, A4R4G4B4) mit.
	//   =1 (661): Neigungsmaske -- flach Sand, steil Fels (so ist
	//       die Selektorkarte gemalt, Korrelation +0.61).
	//   =2 (668): die ECHTE Selektorkarte von s2 abtasten -- die
	//       Materialgrenzen folgen den GEMALTEN Formen der Karte.
	char TM[8] = {};
	if (GetEnvironmentVariableA("AQUANOX_TERRAINMIX", TM, sizeof(TM)) != 0)
	{
		AqTerrainMix = atoi(TM);
		if (AqTerrainMix < 0 || AqTerrainMix > 2)
			AqTerrainMix = 0;
#ifndef D3D8TO9NOLOG
		LOG << "> AquaNox-TERRAINMIX: " << AqTerrainMix
		    << (AqTerrainMix == 2 ? " (Selektorkarte)"
		      : AqTerrainMix == 1 ? " (Neigung)" : " (aus)") << std::endl;
#endif
	}

	// AQUANOX_LICHTER=1: eigene Lichtquellen aus mod_docu\lichter.txt
	// (649). Format je Zeile: x y z  r g b  intensitaet  radius
	// modus(0=fest/1=pulsierend/2=blinkend)  periode_ms; Zeilen mit
	// # sind Kommentare. Position im WELTRAUM -- eingespeist nur in
	// die Prelit-Basis (19) der Statics, deren Objektraum der
	// Weltraum ist (607b); der Detailpass formt das Licht mit (645).
	char LI[8] = {};
	if (GetEnvironmentVariableA("AQUANOX_LICHTER", LI, sizeof(LI)) != 0
		&& LI[0] == '1')
	{
		// 655: LEVELGEBUNDENE Lichtdateien (Stefans Einwand -- eine
		// globale Datei ohne Levelbezug ist falsch). Der Startlevel
		// steht in der Kommandozeile (-s map\<karte>\script\<karte>);
		// geladen wird mod_docu\lichter\<karte>.txt, Rueckfall auf
		// mod_docu\lichter.txt. GRENZE: gilt fuer den STARTLEVEL;
		// Levelwechsel im Prozess brauchen spaeter eine eigene
		// Erkennung (Kandidat: kartenspezifischer Lightmap-Hash).
		char LPfad[MAX_PATH] = "mod_docu\\lichter.txt";
		const char *cmd = GetCommandLineA();
		const char *sArg = cmd != nullptr ? strstr(cmd, "-s ") : nullptr;
		if (sArg != nullptr)
		{
			char arg[192] = {};
			size_t an = 0;
			for (const char *p = sArg + 3;
				*p != '\0' && *p != ' ' && *p != '"' && an + 1 < sizeof(arg); ++p)
				arg[an++] = *p;
			const char *name = arg;
			for (const char *p = arg; *p != '\0'; ++p)
				if (*p == '\\' || *p == '/')
					name = p + 1;
			char probe[MAX_PATH];
			sprintf_s(probe, sizeof(probe), "mod_docu\\lichter\\%s.txt", name);
			FILE *tdatei = nullptr;
			if (fopen_s(&tdatei, probe, "r") == 0 && tdatei != nullptr)
			{
				fclose(tdatei);
				strcpy_s(LPfad, sizeof(LPfad), probe);
			}
		}
		FILE *ldatei = nullptr;
		if (fopen_s(&ldatei, LPfad, "r") == 0
			&& ldatei != nullptr)
		{
			char lzeile[256];
			while (fgets(lzeile, sizeof(lzeile), ldatei) != nullptr
				&& AqLichterZahl < 16)
			{
				if (lzeile[0] == '#')
					continue;
				AqLicht &l = AqLichter[AqLichterZahl];
				if (sscanf_s(lzeile, "%f %f %f %f %f %f %f %f %d %f",
					&l.Pos[0], &l.Pos[1], &l.Pos[2],
					&l.Farbe[0], &l.Farbe[1], &l.Farbe[2],
					&l.Intensitaet, &l.Radius, &l.Modus,
					&l.PeriodeMs) == 10)
					++AqLichterZahl;
			}
			fclose(ldatei);
		}
		AqLichterAn = AqLichterZahl > 0;
		AqLichtTick();
#ifndef D3D8TO9NOLOG
		LOG << "> AquaNox-LICHTER: " << AqLichterZahl
		    << " geladen aus " << LPfad << " (Mehrslot, 19+23)." << std::endl;
#endif
	}
...
void Direct3DDevice8::AqVorZeichnen()
{
	...
	if (Prelit && AqModus != 1)
	{
		...
		// 671: KAUSTIK-GATE-Beweiszeile auch im Prelit-Pfad -- das
		// Gate gilt fuer ALLE uebernommenen Modi (5, 19, 23), nicht
		// nur 5/23: die Messdaten zeigen die Stufen-Vielfalt, und ein
		// Kaustik-Frame kann auch in einer Prelit-Kette haengen.
#ifndef D3D8TO9NOLOG
		if (kn.KaustikStufe != 255)
		{
			const unsigned gm = kn.Modus, gs = kn.KaustikStufe,
				gop = kn.Op[gs][0];
			if (AqTexGesehen.insert((0x6A7EULL << 40)
				^ (static_cast<unsigned long long>(gm) << 16)
				^ (static_cast<unsigned long long>(gs) << 8)
				^ gop).second)
				LOG << "> AquaNox-KAUSTIK-GATE: Modus=" << gm
				    << " St=" << gs << " Op=" << gop << std::endl;
		}
#endif
		...
		if (AqKonstSchmutzig)
		{
			...
			ProxyInterface->SetPixelShaderConstantF(2, AqRelief, 1);
			{
				...
				// 652/654: eigene Punktlichter ab c200, je Zug in den
				// Objektraum verschoben (654).
				AqLichtUpload();
			}
	...
	if (Detail && AqModus != 1)
	{
		AqSchluessel kd;
		AqWaehleKartenquelle();
		const bool KdOk = AqBaueSchluessel(kd) && !kd.Effekt;
#ifndef D3D8TO9NOLOG
		// 670: KAUSTIK-ZUG-DIAGNOSE, Detailpfad. 669 feuerte nie --
		// traegt der Detailzug den Kaustik-Frame auf einer ANDEREN
		// Stufe als 0 (649er-Log: der Station-Detailzug 0b64fc55 hat
		// zwei Stufen mit BLENDTEXTUREALPHA)? BEWUSST VOR der
		// Ablehnungsbehandlung: wird der Kaustik-Zug vom Schluesselbau
		// abgelehnt, laeuft er still als FF weiter -- genau dieser Fall
		// braucht die Zeile am dringendsten (Schl=0). Einmal je
		// (Modus,Stufe,Hash).
		{
			const unsigned long long ZH[4] = { AqHash0,
				AqStufe1Tex != nullptr ? AqStufe1Tex->AqHash : 0,
				AqHash2, AqHash3 };
			for (unsigned zs = 0; zs < 4; ++zs)
				if (ZH[zs] != 0 && AqKaustikHashes.count(ZH[zs]) != 0
					&& AqTexGesehen.insert(ZH[zs] ^ (23ULL << 16)
						^ (static_cast<unsigned long long>(zs) << 8)
						^ 0x26AULL).second)
					LOG << "> AquaNox-KAUSTIK-ZUG: Modus=23 St=" << zs
					    << " Hash=" << std::hex << ZH[zs] << std::dec
					    << " Schl=" << (KdOk ? 1 : 0) << std::endl;
		}
#endif
		if (!KdOk)
		{
		...
		// 671: Die KaustikStufe steht seit dem Schluesselbau im
		// Schluessel (dynamisch, erste Listen-Stufe) -- die feste
		// 669er-St0-Erkennung ist ausgebaut. Beweiszeile KAUSTIK-GATE
		// (ersetzt KAUSTIK-23) einmal je (Modus,Stufe,Op)-Kombination.
#ifndef D3D8TO9NOLOG
		if (kd.KaustikStufe != 255)
		{
			const unsigned gm = kd.Modus, gs = kd.KaustikStufe,
				gop = kd.Op[gs][0];
			if (AqTexGesehen.insert((0x6A7EULL << 40)
				^ (static_cast<unsigned long long>(gm) << 16)
				^ (static_cast<unsigned long long>(gs) << 8)
		...
		if (AqKonstSchmutzig)
		{
			...
			ProxyInterface->SetPixelShaderConstantF(2, AqRelief, 1);
			{
				...
				// 653/654: Licht-Upload auch im 23er (Terrain-
				// Abdeckung), je Zug in den Objektraum verschoben.
				AqLichtUpload();
			}
			AqKonstSchmutzig = false;
		}

		// *** 674: KEIN c7-UPLOAD MEHR FUER DIE VOLLVARIANTE. *** Der
		// 671er-Spiegel samt 673er-Signatur-Filter ist ausgebaut: die
		// Engine laedt NIE eine Selektorskala nach c15/c16 (statische
		// Herleitung, Handoff 674 -- c15/c16 sind die MATERIAL-
		// Fensterparameter, die Selektor-UV ist Vertexdatum v3). Die
		// Vollvariante liest die Selektor-UV jetzt direkt aus dem
		// Vertexstrom (oT4-Durchreichung der dmfm-Zweitfassung,
		// CreateVertexShader; Generator: texld auf t4). ps c7 ist
		// damit wieder frei.
#ifndef D3D8TO9NOLOG
		// Beweiszeile einmal beim ERSTEN ECHTEN Terrainzug (gleiche
		// Signatur wie der TerrainMix-Block des Generators --
		// Stationen laufen auch durch Modus 23): der neue Weg ist
		// aktiv, und die Schrittweite von Stream 0 muss 40 sein
		// (gemeinsame Deklaration der Terrain-Familie aus 0x50b7a0;
		// v3 = FLOAT2 bei Offset 28). Stride != 40 hiesse: die
		// Deklarations-Herleitung stimmt nicht, v3 waere Muell.
		if (AqTerrainMix == 2 && !AqTerrainMix2Gemeldet
			&& kd.Stufen == 1 && kd.NormalKarte != 0
			&& kd.Tex[0] == 1
			&& kd.Op[0][0] == D3DTOP_SELECTARG2)
		{
			AqTerrainMix2Gemeldet = true;
			// oT4=1: die dmfm-Erkennung hat die Durchreichung
			// wirklich angehaengt; oT4=0 hiesse, der PS laese
			// undefiniertes texcoord4 (dann Disassemblat-Wortlaut
			// der dmfm-Erkennung pruefen).
			LOG << "> AquaNox-TERRAINMIX2: Weg=v3 Stride="
			    << AqStrom0Schritt << " (Soll 40) oT4="
			    << (AqSelektorDurchgereicht ? 1 : 0) << std::endl;
		}
#endif
		return;
	}

	// *** DETAILPASS-UEBERNEHMER (614): STEHT WEITER OBEN. *** Hier
	// stand bis 646 ein totes if(false)-Duplikat des Blocks (639:
	// der Grep-Fehlbefund 624 hatte ihn doppelt eingefuegt) --
	// ausgebaut; Wortlaut in mod_docu\hook\d3d8to9_device_s645.cpp.
	...
	AqWaehleKartenquelle();
	const bool KOk = AqBaueSchluessel(k);
#ifndef D3D8TO9NOLOG
	// 670: KAUSTIK-ZUG-DIAGNOSE, Licht-Pfad -- traegt IRGENDEINE
	// Stufe (0..3) des Zugs einen Kaustik-Hash (die 669er-Annahme
	// "Stufe 2" koennte an der falschen Stufe haengen)? BEWUSST VOR
	// der Erfolgsauswertung: auch ein abgelehnter Zug (Schl=0, faellt
	// auf FF) wird gemeldet. Einmal je (Modus,Stufe,Hash); Modus =
	// AqModus, denn genau den traegt k im Licht-Pfad (Z. 4657).
	{
		const unsigned long long ZH[4] = { AqHash0,
			AqStufe1Tex != nullptr ? AqStufe1Tex->AqHash : 0,
			AqHash2, AqHash3 };
		for (unsigned zs = 0; zs < 4; ++zs)
			if (ZH[zs] != 0 && AqKaustikHashes.count(ZH[zs]) != 0
				&& AqTexGesehen.insert(ZH[zs]
					^ (static_cast<unsigned long long>(AqModus) << 16)
					^ (static_cast<unsigned long long>(zs) << 8)
					^ 0x26AULL).second)
				LOG << "> AquaNox-KAUSTIK-ZUG: Modus=" << AqModus
				    << " St=" << zs
				    << " Hash=" << std::hex << ZH[zs] << std::dec
				    << " Schl=" << (KOk ? 1 : 0) << std::endl;
	}
#endif
	if (KOk)
	{
		// 671: Die KaustikStufe kommt dynamisch aus dem Schluesselbau
		// (erste Kettenstufe mit Listen-Hash) -- die feste 669er-
		// Annahme "Schiffs-Kaustik IST Stufe 2" ist ausgebaut: der
		// 23:49-Messlauf zeigte Modus-5-Zuege mit Kaustik auf St=0,
		// St=1 UND St=2; das feste S2-Gate dimmte beim Bomber die
		// FALSCHE Stufe (Stefans Regression). Beweiszeile
		// KAUSTIK-GATE (ersetzt KAUSTIK-S2) einmal je Kombination.
		// (670-Reihenfolge-Befund bleibt gueltig: k.Modus ist hier
		// gesetzt, der Licht-Pfad laesst AqModus stehen.)
#ifndef D3D8TO9NOLOG
		if (k.KaustikStufe != 255)
		{
			const unsigned gm = k.Modus, gs = k.KaustikStufe,
				gop = k.Op[gs][0];
			if (AqTexGesehen.insert((0x6A7EULL << 40)
				^ (static_cast<unsigned long long>(gm) << 16)
				^ (static_cast<unsigned long long>(gs) << 8)
...
bool Direct3DDevice8::AqBaueSchluessel(AqSchluessel &k) const
{
	...
	// *** 255 = KEINE KAUSTIK, NICHT 0. *** Das memset laesst 0
	// stehen, und 0 ist seit 671 eine GUELTIGE Stufennummer --
	// ohne diese Zeile truege jeder Zug ein Stufe-0-Gate.
	k.KaustikStufe = 255;
	...
	// *** KAUSTIK-STUFE DYNAMISCH (671). *** Der 23:49-Messlauf
	// bewies: die 32 Kaustik-Frames binden an St=0 UND St=1 UND
	// St=2 (KAUSTIK-BIND je 32x), und die Zuege tragen sie auf
	// wechselnden Stufen (KAUSTIK-ZUG: Modus=5 St=0/1/2, Modus=23
	// St=1/2, alle Schl=1). Die festen 669er-Annahmen (St0 bzw.
	// St2) dimmten deshalb beim Bomber die FALSCHE Stufe --
	// Texturmischung kippte (Stefans Regression). Jetzt: die ERSTE
	// Kettenstufe, deren Hash in der Liste steht, wandert in den
	// Schluessel; das Generator-Gate haengt exakt an ihr. Gilt fuer
	// ALLE uebernommenen Modi (5, 19, 23) -- die Stufen-Schatten
	// AqHash0/AqStufe1Tex->AqHash/AqHash2/AqHash3 existieren seit
	// s670 vollstaendig. Nur 2D-Stufen (k.Tex==1): nur fuer die
	// laeuft im Generator ueberhaupt ein texld r6.
	if (!AqKaustikHashes.empty())
	{
		for (unsigned s = 0; s < k.Stufen; ++s)
		{
			...
			switch (s)
			{
			case 0: h = AqHash0; break;
			case 1: h = (AqStufe1Tex != nullptr) ? AqStufe1Tex->AqHash : 0; break;
			case 2: h = AqHash2; break;
			case 3: h = AqHash3; break;
			}
			if (h != 0 && AqKaustikHashes.count(h) != 0)
			{
				k.KaustikStufe = static_cast<unsigned char>(s);
				break;
			}
		}
	}
...
std::string Direct3DDevice8::AqBaueQuelltext(const AqSchluessel &k) const
{
	...
	else
	{
		...
		if (k.Modus == 5 || k.Modus >= 14)
		{
			...
			else if (AqParallaxAn && k.OrmKarte)
			{
				...
				Rumpf += "    add r21.w, r21.x, -c31.y\n"
				         ...
				         // Streifblick-Verstaerkung (644): /max(V.N, 0.5).
				         // Ohne die Division ist Parallaxe genau dort am
				         // schwaechsten, wo sie am auffaelligsten waere
				         // (flacher Blick ueber die Flaeche). Klemme 0.5 =
				         // hoechstens Faktor 2, keine Ausreisser an der
				         // Silhouette. r0 ist hier die UNGESTOERTE Normale.
				         "    dp3 r21.z, r7, r0\n"
				         "    max r21.z, r21.z, c31.y\n"
				         "    rcp r21.z, r21.z\n"
				         "    dp3 r21.x, r7, r8\n"
				         "    dp3 r21.y, r7, r9\n"
				         "    mul r21.xy, r21, r21.w\n"
				         "    mul r21.xy, r21, r21.z\n";
		...
		if (k.Modus >= 10 || k.Modus == 5 || k.Modus >= 14)
		{
			...
			// Das eigene Punktlicht (649) stand bis 650 HIER auf r2 --
			// WIRKUNGSLOS: exakt die dokumentierte 594b-Falle direkt
			// aus dem Nachbar-Kommentar oben (die Statics-Basis-Ketten
			// lesen DIFFUSE nicht; TFACTOR mal Textur mal Lightmap).
			// Es steht jetzt wie das Wandlicht am ENDE der Kette auf
			// r3 (651). LEHRE: Vor jeder Einfuegung die NACHBAR-
			// Kommentare der Stelle lesen -- der Fehler samt Loesung
			// stand woertlich daneben.
	...
	for (unsigned s = 0; s < k.Stufen; ++s)
	{
		...
		// *** KAUSTIK-GATE MIT DYNAMISCHER STUFE (671). *** Der
		// 23:49-Messlauf bewies: die Kaustik-Stufe ist JE ZUG
		// VERSCHIEDEN (Modus=5 St=0/1/2, Modus=23 St=1/2) -- das
		// Dimmen sitzt deshalb DIREKT am texld der Traegerstufe
		// (s == k.KaustikStufe), nicht mehr fest an St2 (Schiff)
		// oder am Rumpfende (Statics-Ganzzug). Und es sitzt am
		// TEXEL r6, OP-ABHAENGIG am Neutralwert der Stufe:
		//   BLENDTEXTUREALPHA: Alpha steuert die Mischung -> r6.w*=f
		//     (Neutral 0: lrp gibt CURRENT zurueck).
		//   MODULATE:   Farbe multipliziert -> Neutral 1.0:
		//     r6.rgb = 1 + (r6.rgb - 1)*f.
		//   MODULATE2X: Neutral 0.5 (2*d*0.5 = d):
		//     r6.rgb = 0.5 + (r6.rgb - 0.5)*f. NIEMALS gegen 0 --
		//     das malte schwarze Flaechen (669er-Lehre).
		//   ADD und Stufe-0-SELECTARG (Kaustik als Basis, z. B.
		//     additive Kaustik-Overlays): Neutral 0 -> r6.rgb *= f.
		//   MODULATEALPHA_ADDCOLOR (673; Op 18, die haeufigste Op
		//     ueberhaupt -- Schiffs-Grundmischung "S0 MODULATEALPHA_
		//     ADDCOLOR(DIFFUSE,TEXTURE)", 670er-Forensik. Das 671er-
		//     Gate liess sie konservativ aus; deshalb wirkte die
		//     Orientierung auf Schiffen NIRGENDS -- der 00:34-Lauf
		//     meldete KAUSTIK-OP-UNBEKANNT Op=18). HERLEITUNG:
		//     result = Arg1.RGB + Arg1.A * Arg2.RGB (Nachbau unten
		//     im Stufen-Switch: mad_sat r3.xyz, A1.w, A2, A1). Ist
		//     der TEXTURE-Texel r6 das Arg2 (der gemessene Fall,
		//     Kette im Schluessel k.Op geprueft), ist sein GESAMTER
		//     Beitrag der Summand Arg1.A*r6.rgb -> r6.rgb *= f VOR
		//     der Formel; bei f=0 bleibt exakt Arg1.RGB stehen
		//     (Neutral 0). Ist r6 das Arg1 (r6.rgb + r6.a*Arg2.rgb),
		//     dimmt derselbe Griff den direkten Add-Anteil, waehrend
		//     r6.w unangetastet Arg2 weitermoduliert. BEIDE Lagen:
		//     Griff wie ADD (r6.rgb *= f), r6.w NIE anfassen.
		// UNDOKUMENTIERTE Ops (SELECTARG jenseits Stufe 0, weitere
		// ungemessene): Gate NICHT anwenden (konservativ, Zug bleibt
		// unveraendert) + Einmal-Logzeile KAUSTIK-OP-UNBEKANNT --
		// NICHT raten (673).
		// Up-Vektor je Modus: Modus 5 -> c13 (Hemi-Achse, je Objekt
		// im Objektraum gesetzt, rollfest); Modus 19/23 -> frisch
		// normalisierte t5-Normale, N.z (Statics sind nur verschoben,
		// Objektraum-Nz == Welt-Nz; c13 dort veraltet, 669er-Befund).
		// f = (1-s) + s*sat(up-Anteil); s/(1-s) als def-Literale in
		// c32 (Kopf). AQUANOX_KAUSTIK=0 -> f fest 0 (op-abhaengiger
		// Neutralwert direkt). Nur ps_3_0-Familie (r27/r28 existieren
		// in ps_2_0 nicht). mad-Ketten statt lrp (X5766); je
		// Anweisung EIN Konstantenregister (X5584; c32.x/c32.y sind
		// DASSELBE Register).
		if (s == k.KaustikStufe && k.Tex[s] == 1
			&& ((k.Modus >= 5 && k.Modus <= 9) || k.Modus >= 14))
		{
			const bool SelectTex0 = s == 0
				&& ((COp == D3DTOP_SELECTARG1 && CA1 == D3DTA_TEXTURE)
				 || (COp == D3DTOP_SELECTARG2 && CA2 == D3DTA_TEXTURE));
			const bool OpBekannt = COp == D3DTOP_BLENDTEXTUREALPHA
				|| COp == D3DTOP_MODULATE || COp == D3DTOP_MODULATE2X
				|| COp == D3DTOP_ADD || SelectTex0
				|| COp == D3DTOP_MODULATEALPHA_ADDCOLOR;
...
#ifndef D3D8TO9NOLOG
				char zk[96];
				sprintf_s(zk, sizeof(zk), "KaustikOp=%u St%u M%u", COp, s,
					static_cast<unsigned>(k.Modus));
				if (AqWarum.insert(std::string(zk)).second)
					LOG << "> AquaNox-KAUSTIK-OP-UNBEKANNT: Op=" << COp
					    << " St=" << s << " Modus="
					    << static_cast<unsigned>(k.Modus) << std::endl;
#endif
			}
			else if (AqKaustikAus)
			{
				// f = 0 fest: der op-abhaengige Neutralwert direkt.
				if (COp == D3DTOP_BLENDTEXTUREALPHA)
					Rumpf += "    mov r6.w, c32.w\n";
				else if (COp == D3DTOP_MODULATE)
					Rumpf += "    mov r6.xyz, c31.w\n";
				else if (COp == D3DTOP_MODULATE2X)
					Rumpf += "    mov r6.xyz, c31.y\n";
				else   // ADD / Stufe-0-SELECTARG / Op 18 (673)
					Rumpf += "    mov r6.xyz, c32.w\n";
			}
			else
			{
				// f = (1-s) + s*sat(up-Anteil) -> r27.w.
				if (k.Modus == 5)
					Rumpf += "    dp3 r27.w, t5, t5\n"
					         "    max r27.w, r27.w, c31.x\n"
					         "    rsq r27.w, r27.w\n"
					         "    mul r27.xyz, t5, r27.w\n"
					         "    dp3_sat r28.w, r27, c13\n"
					         "    mad r27.w, r28.w, c32.x, c32.y\n";
				else
					Rumpf += "    dp3 r27.w, t5, t5\n"
					         "    max r27.w, r27.w, c31.x\n"
					         "    rsq r27.w, r27.w\n"
					         "    mul_sat r28.w, t5.z, r27.w\n"
					         "    mad r27.w, r28.w, c32.x, c32.y\n";

				if (COp == D3DTOP_BLENDTEXTUREALPHA)
					Rumpf += "    mul r6.w, r6.w, r27.w\n";
				else if (COp == D3DTOP_MODULATE)
					Rumpf += "    add r28.xyz, r6, -c31.w\n"
					         "    mad r6.xyz, r28, r27.w, c31.w\n";
				else if (COp == D3DTOP_MODULATE2X)
					Rumpf += "    add r28.xyz, r6, -c31.y\n"
					         "    mad r6.xyz, r28, r27.w, c31.y\n";
				else   // ADD / Stufe-0-SELECTARG / Op 18 (673)
					Rumpf += "    mul r6.xyz, r6, r27.w\n";
			}
		}
		...
		case D3DTOP_BLENDTEXTUREALPHA:
			// Das 669er-Schiffs-Kaustik-Gate (fest an s == 2) stand
			// HIER -- seit 671 ausgebaut: das dynamische Stufen-Gate
			// oben (direkt nach dem texld der Traegerstufe) hat r6.w
			// bereits gedimmt, wenn DIESE Stufe den Kaustik-Frame
			// traegt. Die feste St2-Annahme dimmte beim Bomber die
			// FALSCHE Stufe (Messlauf 671: Stufe je Zug verschieden).
	...
	// *** TERRAIN-NEIGUNGSMIX (661). *** Der Terrain-Detailzug ist
	// 1-stufig SELECTARG2(TEXTURE) mit material_3 (Kies) auf s0 --
	// die Engine bindet material_0 (Sand) auf St1 und material_1
	// (Fels) auf St3 MIT, nutzt sie aber nicht (660: Kette endet
	// nach Stufe 1; die Vielfalt steckt in den Basis-Splatlagen).
	// Hier werden die gebundenen PBR-Ersaetze mitgesampelt und per
	// NEIGUNG gemischt: flach -> Sand, steil -> Fels (so haben die
	// Entwickler die Selektorkarte gemalt). UV-Skalen aus
	// material.des (1h1: W0=110, W1=32, W3=8 Welteinheiten je
	// Kachel -> Faktoren 8/110 und 8/32 relativ zu s0; def c6 --
	// c6/c7 sind seit dem 652er-Umzug der Lichter auf c200 frei).
	// r20 = UNGESTOERTE Normale (bei Modus 23 gerettet), Z = oben.
	// Maske = sat(4*(1-Nz) - 0.5): voll Sand bis ~29 Grad Hang,
	// voll Fels ab ~52 Grad. Ergebnis halb/halb gegen den Kies
	// (lrp c31.y), damit die Makroschicht lesbar bleibt. r24/r25
	// werden gleich danach vom Anti-Kachel-Block wiederverwendet.
	// *** VOLLVARIANTE (668, AQUANOX_TERRAINMIX=2): DIE ECHTE
	// SELEKTORKARTE. *** Auf St2 haengt beim Detailzug der engine-
	// gebaute Slot-1-Selektor (FELS, A4R4G4B4; Dump 633624ac ==
	// flipud(Slot-1-Maske 0x10<=A<0x30) zu 100,00% verifiziert) --
	// gebunden, aber ketten-inaktiv.
	// *** SEIT 674: DIE SELEKTOR-UV IST EIN VERTEXDATUM (v3). ***
	// Die statische Herleitung (Handoff 674) korrigierte die 668er-
	// Formelzuordnung: (v0.xy+c15)*c16 ist die MATERIAL-UV der
	// mmsm-Familie (c16 = 1/W aus material.des), die Selektor-UV
	// dagegen schreibt der mmsm-VS als "mov oT0.xy, v3" DIREKT aus
	// dem Vertexstrom -- die Engine backt welt/4096 je Vertex ein
	// (Neuverankerungen aendern v3 nicht falsch: das Terrain wird
	// je Bild neu trianguliert, v3 konsistent mitberechnet -> die
	// Maske ist springfrei, ohne Anker-Arithmetik). Der 671er-
	// c15/c16-Spiegel samt 673er-Filter jagte ein Phantom (sig=0
	// im 00:52-Log: NIE ein Upload in "Selektorskala") und ist
	// ausgebaut. Die dmfm-Zweitfassung reicht v3 als oT4 durch
	// (CreateVertexShader, byte-exakte dmfm-Erkennung); hier wird
	// mit t4 direkt abgetastet. KEIN V-Flip (668-Befund).
	// Gewicht Fels = Alpha der Hartmaske (0xF -> 1.0), an den
	// Kanten durch die bilineare Filterung von selbst weich;
	// Sand = 1 - Fels. Einschraenkung: auf den 5-6 Karten mit
	// Slot-2-Anteilen (A>=0x30) faellt Slot 2 in den Sand-Anteil.
	const bool TerrainMix = AqTerrainMix != 0 && k.Modus == 23
		&& k.Stufen == 1 && k.NormalKarte && k.Tex[0] == 1
		&& k.Op[0][0] == D3DTOP_SELECTARG2;
	if (TerrainMix)
	{
		sprintf_s(z, sizeof(z),
			"    mul r24.xy, t%u, c6.x\n"
		...
		Rumpf += z;
		if (AqTerrainMix == 2)
			// Vollvariante (674): Fels-Gewicht aus der Selektorkarte,
			// UV = t4 = die ORIGINALE Selektor-UV der Engine
			// (Vertexdatum v3, per oT4 durchgereicht -- Herleitung im
			// Blockkommentar oben). r26 wird gleich vom Misch-Schwanz
			// wiederverwendet -- das Alpha ist dann laengst in r23.w
			// gerettet.
			Rumpf += "    texld r26, t4, s2\n"
			         "    mov r23.w, r26.w\n";
		else
			// Neigungsheuristik (661): Maske aus der ungestoerten
			// Normale r20 (Z = oben).
			Rumpf += "    add r23.w, c31.w, -r20.z\n"
			         "    mul r23.w, r23.w, c31.z\n"
			         "    mul r23.w, r23.w, c31.z\n"
			         "    add_sat r23.w, r23.w, -c31.y\n";
		// Kein lrp: X5766 verbietet Ziel = erste/dritte Quelle
		// (599-Probe fing es) -- mad-Ketten stattdessen.
		Rumpf += "    add r26.xyz, r25, -r24\n"
		         "    mad r26.xyz, r23.w, r26, r24\n"
		         "    mul r26.xyz, r26, c31.y\n"
		         "    mad r3.xyz, r3, c31.y, r26\n";
	}

	// *** ANTI-KACHEL (649): Grossskala-Selbstmodulation. *** Die
	// sichtbare Textur wird ZUSAETZLICH in ~30x groesserer Welt-
	// Skala abgetastet (Schraegsummen-Projektion wie 607b, c5.z);
	// die Luma-DIFFERENZ gross-minus-eigen moduliert um Faktor 1.0
	// (Erwartungswert 0 -> Gesamthelligkeit bleibt, materialneutral)
	// und bricht das Wiederholungsraster in der Ferne (Stefans
	// Sediment-Befund 649). fp32 unkritisch: UV-Magnitude ~20 (das
	// 618er-Problem begann bei ~1500). c1.x (1/pi) dient als
	// Beinahe-Drittel fuer die Luma-Summe -- der kleine Fehler
	// kuerzt sich in der Differenz heraus.
	if (k.Modus == 23 && AqAntikachelAn)
	{
		// *** SPRUNGKOMPENSIERTE BASIS AUF DEM TERRAIN (673/674). ***
		// Die rohen t6-Schraegsummen springen auf dem Terrain bei
		// der Sektor-Neuverankerung (t6 = v0 durchgereicht) -- das
		// Anti-Kachel-Muster ruckte im Sektortakt mit (Stefans
		// Beleuchtungs-Ruckeln beim Fliegen). Die 673er-Kompensation
		// ueber c7 = (c15.xy, c16.xy) war WIRKUNGSLOS (der Filter
		// liess nie Werte durch, c7 blieb 0 = roher Weg). Seit 674
		// liefert t4 (Selektor-UV, Vertexdatum) die ABSOLUTE
		// Weltposition: welt.xy = t4 * 4096 (c6.z; 668-Abbildung
		// u = x/4096 ohne V-Flip) -- springfrei, weil die Engine v3
		// bei jeder Neutriangulierung konsistent mitrechnet. z
		// bleibt ROH aus t6 (Hoehe; springt sie bei der Neuveranke-
		// rung doch, bliebe ein Rest-Ruckeln -- dokumentiert offen).
		// X5584: c6 und c5 je in eigener Anweisung. X5583 (599-Probe
		// pruef_674 fing es): auch je Anweisung nur EIN v-Eingangs-
		// register -- t4 und t6 muessen in getrennte Zeilen, daher
		// mul + add/add statt zweier mads. Nicht-Terrain-23er (kein
		// TerrainMix-Schluessel, kein t4 im Shader): roher Weg
		// unveraendert -- Stationsurspruenge sind stabil.
		if (TerrainMix && AqTerrainMix == 2)
			Rumpf += "    mul r24.xy, t4, c6.z\n"
			         "    add r24.x, r24.x, t6.z\n"
			         "    add r24.y, r24.y, -t6.z\n"
			         "    mul r24.xy, r24, c5.z\n";
		else
			Rumpf += "    add r24.x, t6.x, t6.z\n"
			         "    add r24.y, t6.y, -t6.z\n"
			         "    mul r24.xy, r24, c5.z\n";
		sprintf_s(z, sizeof(z), "    texld r24, r24, s%u\n",
			static_cast<unsigned>(k.KartenStufe));
		Rumpf += z;
		Rumpf += "    dp3 r24.w, r24, c1.x\n"
		         "    dp3 r25.w, r6, c1.x\n"
	...
	// *** EIGENE PUNKTLICHTER (649/651/652): PRELIT-BASIS. ***
	// Je Licht 2 Register ab c200: Pos + 1/R^2, Farbe*Intensitaet
	// (Puls/Blinken rechnet die CPU, AqLichtTick). N.L auf der
	// GESTOERTEN Normale r0 (Relief antwortet); Falloff
	// (1-d^2/R^2)^2. ADDITIV auf r3 am KETTENENDE -- der erste
	// Standort auf r2 war die 594b-Falle (Basis-Ketten lesen
	// DIFFUSE nicht). NACH dem Wandlicht: der Praegefaktor soll
	// den Schein nicht mitmodulieren; der Detailpass desselben
	// Objekts formt ihn ohnehin mit (645). AqLichterZahl ist
	// prozesskonstant -- kein Schluesselfeld noetig; die Schleife
	// wird je Licht entrollt (14 Anweisungen pro Licht).
	// *** AUCH MODUS 23 (653): *** Die 19er-Prelit-Zuege decken nur
	// die STATIONSWAENDE ab (Log-Befund: alle PRELIT-ZUG-Hashes sind
	// Wandtexturen) -- Terrain und Kisten um die Masten laufen dort
	// NIE durch. Der 23er-Detailpass traegt die sichtbare Textur
	// ALLER Weltflaechen; sein 2*src*dest-Blend multipliziert den
	// additiven r3-Anteil mit der Basisfarbe (rot-getoente
	// Aufhellung -- gewollt; Intensitaet dafuer in lichter.txt
	// angehoben). Auf Flaechen mit BEIDEN Passen wirkt das Licht
	// doppelt (mild, da der 23er-Anteil basisgedaempft ist).
	if ((k.Modus == 19 || k.Modus == 23) && AqLichterAn)
		for (int li = 0; li < AqLichterZahl && li < 16; ++li)
		{
			sprintf_s(z, sizeof(z),
				"    add r22.xyz, c%d, -t6\n"
				"    dp3 r22.w, r22, r22\n"
				"    mul r23.w, r22.w, c%d.w\n",
				200 + 2 * li, 200 + 2 * li);
			Rumpf += z;
			Rumpf += "    add r23.w, c31.w, -r23.w\n"
			         "    max r23.w, r23.w, c31.x\n"
			         "    mul r23.w, r23.w, r23.w\n"
			         "    max r22.w, r22.w, c31.x\n"
			         "    rsq r22.w, r22.w\n"
			         "    mul r22.xyz, r22, r22.w\n"
			         "    dp3 r22.w, r0, r22\n"
			         "    max r22.w, r22.w, c31.x\n"
			         "    mul r22.w, r22.w, r23.w\n";
			sprintf_s(z, sizeof(z),
				"    mul r23.xyz, c%d, r22.w\n", 201 + 2 * li);
			Rumpf += z;
			Rumpf += "    add r3.xyz, r3, r23\n";
		}
	...
	// Das 669er-Statics-Ganzzug-Gate (k.Kaustik == 1: r3 gegen 0.5
	// am Rumpfende) stand HIER -- seit 671 ausgebaut: das dynamische
	// Stufen-Gate sitzt jetzt IN der Stufen-Nachbau-Schleife direkt
	// am texld der Traegerstufe und dimmt op-abhaengig am richtigen
	// Neutralwert. Ein Ganzzug-Dimmen haette auch Nicht-Kaustik-
	// Stufen desselben Zugs (Material-Mischungen!) mitorientiert.
	...
	if ((k.Modus >= 5 && k.Modus <= 9) || k.Modus >= 14)
	{
		...
		// auseinanderlaufen.
		static const char *const Tausch[8][2] = {
			{ "t0", "v2" }, { "t1", "v3" }, { "t2", "v4" }, { "t3", "v5" },
			{ "t5", "v6" }, { "t6", "v7" }, { "t7", "v8" },
			// 674: t4 = Selektor-UV der TerrainMix-Vollvariante
			// (oT4-Durchreichung der dmfm-Zweitfassung). v9 ist
			// frei (v2..v8 belegt, s.o.); Kopf deklariert
			// dcl_texcoord4 v9 nur, wenn der Rumpf t4 nutzt.
			{ "t4", "v9" },
		};
		...
		// 669/671: Kaustik-Orientierung -- s und (1-s) als def-Literale
		// in den Kopf gebacken (prozesskonstant, sprintf %.6f).
		// c32 ist frei: Kamera c0, GGX c1, Regler c2, defs c1/c3/
		// c6/c30/c31, AqKonst c8..c29, Lichter ab c200 -- zur
		// Laufzeit schreibt nichts auf c32. w = 0.0 traegt das
		// harte Aus fuer AQUANOX_KAUSTIK=0.
		if (k.KaustikStufe != 255)
		{
			sprintf_s(z, sizeof(z), "    def c32, %.6f, %.6f, 0.5, 0.0\n",
				AqKaustikRichtung, 1.0f - AqKaustikRichtung);
			Kopf += z;
		}
		...
		// Terrain-Neigungsmix (661): Sand (St1) und Fels (St3) sind
		// von der Engine gebunden, aber ketten-inaktiv -- NUR die
		// Sampler deklarieren, KEINE dcl_texcoord (der VS liefert
		// die Saetze nicht; gesampelt wird mit skalierten
		// KartenStufen-UVs). c6 = UV-Faktoren 8/110 und 8/32
		// (material.des 1h1; Ausbau: je Karte von der CPU laden);
		// c6.z = 4096.0 (674): Kantenlaenge der Karte in WE --
		// welt.xy = t4 * c6.z fuer die Anti-Kachel-Basis (668:
		// Selektor-UV = welt/4096, Umkehrung exakt).
		if (TerrainMix)
		{
			Kopf += "    def c6, 0.0727273, 0.25, 4096.0, 0.0\n"
			        "    dcl_2d s1\n"
			        "    dcl_2d s3\n";
			// Vollvariante (668/674): der Fels-Selektor liegt auf s2
			// (ketten-inaktiv gebunden; Stufen == 1 garantiert,
			// dass die TexBenutzt-Schleife s2 nie selbst anlegt).
			// t4 = Selektor-UV aus der dmfm-Zweitfassung (oT4/
			// texcoord4); im ps_3_0-Tausch weiter unten wird t4 zu
			// v9 -- die Deklaration muss hier mit.
			if (AqTerrainMix == 2)
				Kopf += "    dcl_2d s2\n"
				        "    dcl_texcoord4 v9\n";
		}
