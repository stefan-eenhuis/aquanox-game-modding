...
private:
	...
	// Kaustik-Orientierung (669, dynamisch seit 671): 255 = kein
	// Kaustik-Frame im Zug, sonst 0..3 = die ERSTE Kettenstufe,
	// deren Textur-Hash in der Kaustik-Liste steht. Der Messlauf
	// 671 (23:49) bewies: die Kaustik-Stufe ist JE ZUG VERSCHIEDEN
	// (Modus=5 St=0/1/2, Modus=23 St=1/2) -- die festen 669er-
	// Annahmen (St0 bzw. St2) dimmten beim Bomber die FALSCHE
	// Stufe. Muss in den Schluessel: derselbe Kettenaufbau
	// existiert mit und ohne Kaustik-Frame -- ohne Feld bekaemen
	// beide denselben Shader.
	unsigned char KaustikStufe;
	...
	float AqVSK[8][4] = {};
	// 674: Der 671er-c15/c16-Schatten ist AUSGEBAUT. Die statische
	// Herleitung an den Kachel-Orchestratoren (0x4fbfe0/0x4fd550,
	// Helfer 0x4fd340; Handoff 674) zeigte: c15/c16 tragen IMMER
	// die MATERIAL-Fensterparameter (c16 = 1/W mit W aus
	// material.des, 1h1: 110/32; c15 = -floor(S/W)*W), NIE eine
	// Selektorskala -- die Selektor-UV der Terrain-Familie ist ein
	// VERTEXDATUM (v3, "mov oT0.xy, v3" im mmsm-VS). Die
	// TERRAINMIX=2-Vollvariante liest sie seither direkt aus dem
	// Vertexstrom (oT4-Durchreichung der dmfm-Zweitfassung).
	// Diagnose dazu: Schrittweite von Stream 0 (Solldeklaration
	// der Terrain-Familie: 40 Byte, v3 = FLOAT2 bei Offset 28) und
	// ob die dmfm-Erkennung die oT4-Zeile wirklich angehaengt hat
	// (sonst laese der TerrainMix2-PS undefiniertes texcoord4 --
	// der stille Fehlerfall wird so in der Logzeile sichtbar).
	UINT AqStrom0Schritt = 0;
	bool AqSelektorDurchgereicht = false;
	...
	//   2 = Vollvariante (668/671): ECHTE Selektorkarte von s2,
	//       Gewicht = Alpha, UV = (t6.xy + c7.xy) * c7.zw mit
	//       c7 = gespiegelte Engine-Konstanten (c15.xy, c16.xy) --
	//       exakt die Engine-Formel, kein Kamera-Umweg mehr (671).
	int AqTerrainMix = 0;
